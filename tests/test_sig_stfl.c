 ...
 * LMS Test Vector
 */
static OQS_STATUS test_testvector_1(void) {
    ...
    OQS_SIG_STFL *sig = NULL;
    sig = OQS_SIG_STFL_new("LMS_SHA256_H5_W8");
    if (sig == NULL) {
        fprintf(stderr, "ERROR: OQS_SIG_STFL_new failed, Test vector 1.\n");
        goto err;
    }

    rc = OQS_SIG_STFL_verify(sig, message, sizeof(message), signature, sizeof (signature), public_key);
    if (rc != OQS_SUCCESS) {
        fprintf(stderr, "ERROR: Test Vector 1 Verify failed\n");
    }
err:
    OQS_SIG_STFL_free(sig);
    return rc;
...
static OQS_STATUS test_testvector_2(void) {
    ...
    OQS_SIG_STFL *sig = NULL;
    sig = OQS_SIG_STFL_new("LMS_SHA256_H10_W4");
    if (sig == NULL) {
        fprintf(stderr, "ERROR: OQS_SIG_STFL_new failed, Test vector 2.\n");
        goto err;
    }

    rc = OQS_SIG_STFL_verify(sig, message, sizeof(message), signature, sizeof (signature), public_key);
    if (rc != OQS_SUCCESS) {
        fprintf(stderr, "ERROR: Test Vector 2 Verify failed\n");
    }
err:
    OQS_SIG_STFL_free(sig);
    return rc;
...
static OQS_STATUS test_testvector(void) {
    OQS_STATUS rc, rc_1;
    rc = test_testvector_1();
    rc_1 = test_testvector_2();
    if ((rc != OQS_SUCCESS) || (rc_1 != OQS_SUCCESS)) {
        fprintf(stderr, "ERROR: Test Vector 2 Verify failed\n");
        return OQS_ERROR;
    }
    return rc;
...
int main(int argc, char **argv) {
	...
	if (0 == strcasecmp(alg_name, "lms_test_vector")) {
		OQS_STATUS rc = test_testvector();
		OQS_destroy();
		if (rc != OQS_SUCCESS) {
		    printf("Stateful signature algorithm vector test FAILED!\n");
		    return EXIT_FAILURE;
		} else {
	  	  printf("Stateful signature algorithm vector test passed!\n");
	 	   return EXIT_SUCCESS;
	    }
	}
