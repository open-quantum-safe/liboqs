...
// Added new derand integration test
static OQS_STATUS kem_test_derand_correctness(const char *method_name) {
	...
	shared_secret_e = malloc(kem->length_shared_secret + 2 * sizeof(magic_t));
	shared_secret_d = malloc(kem->length_shared_secret + 2 * sizeof(magic_t));
    // Need to define length_coins in 'kem_kyber_size.c'
    // Throw error if length_coins doesn't exist for the requested kem method?
	coins_k = malloc(kem->length_coins + 2 * sizeof(magic_t));
	coins_e = malloc(kem->length_coins + 2 * sizeof(magic_t));
	...
	memcpy(shared_secret_e + kem->length_shared_secret, magic.val, sizeof(magic_t));
	memcpy(shared_secret_d + kem->length_shared_secret, magic.val, sizeof(magic_t));
	memcpy(coins_k + kem->length_coins, magic.val, sizeof(magic_t));
	memcpy(coins_e + kem->length_coins, magic.val, sizeof(magic_t));
...
cleanup:
    ...
    if (coins_k) {
		OQS_MEM_secure_free(coins_k - sizeof(magic_t), kem->length_coins + 2 * sizeof(magic_t));
    }
    if (coins_e) {
		OQS_MEM_secure_free(coins_e - sizeof(magic_t), kem->length_coins + 2 * sizeof(magic_t));
    }
	OQS_KEM_free(kem);
