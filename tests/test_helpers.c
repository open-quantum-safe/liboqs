...
static void hqc_prng_set_state(const void *in) {
    OQS_SHA3_shake256_inc_ctx_clone(&hqc_prng_state, (OQS_SHA3_shake256_inc_ctx *)in);
}

static void hqc_prng_free(void *saved_state) {
    OQS_SHA3_shake256_inc_ctx *hqc_saved_state = (OQS_SHA3_shake256_inc_ctx *)saved_state;
    if (hqc_saved_state != NULL) {
        OQS_SHA3_shake256_inc_ctx_release(hqc_saved_state);
    }
...
static void nist_drbg_free(void *saved_state) {}
