...
IF %COMPILER%==cygwin (
    ...
    c:\cygwin64\bin\bash.exe -lc "echo $OPENSSL_ROOT_DIR && unset OPENSSL_ROOT_DIR && echo $PATH && setup-x86_64.exe -qnNdO -R C:/cygwin64 -l C:/cygwin/var/cache/setup -P openssl-devel -P openssl -P ninja -P cmake -P gcc && cd ${APPVEYOR_BUILD_FOLDER} && openssl version && cygcheck -c && cygcheck -l openssl && cygcheck -l openssl-devel && echo OSSLRD && echo $OPENSSL_ROOT_DIR && find /usr -name opensslv.h -print && pwd && export OPENSSL_ROOT_DIR=/usr/share && echo OSSLRD2 && echo $OPENSSL_ROOT_DIR && mkdir build && cd build && whoami && touch CMakeDoxyfile.tpl && ls -la && cmake .. -GNinja -DCMAKE_C_COMPILER=gcc -DOQS_MINIMAL_BUILD=ON -DOQS_DIST_BUILD=ON -DOQS_ENABLE_SIG_SPHINCS=OFF -DOQS_ENABLE_SIG_RAINBOW=OFF -DBUILD_SHARED_LIBS=%BUILD_SHARED% -DOQS_USE_OPENSSL=%OQS_USE_OPENSSL% && ninja "
)
IF %COMPILER%==msys2 (
    @echo on
