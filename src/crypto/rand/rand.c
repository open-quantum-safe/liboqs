...
double OQS_RAND_zscore_deviation_from_uniform(const unsigned long occurrences[256]) {
	double quantiles[102] = {
		...
		371.1692, 374.0664, 376.9782, 379.9045, 382.8454, 385.8010
	};   // -5.05 to +5.05 sigma: qchisq(pnorm(seq(-5.05,5.05,length.out=102)),255)
	unsigned long total;
	double chsq;
	int i;

	for (total=i=0; i<256; i++) total += occurrences[i];
	if (total/256. < 5) return ZSCORE_SPARSE;

	for (chsq=i=0; i<256; i++) chsq += pow(occurrences[i]-total/256.,2) * 256./total;

	if (chsq <= quantiles[0]) return ZSCORE_BIGNEG;
	for (i=1; i<102; i++) if (chsq <= quantiles[i]) return (i-51) / 10.0;
...
void OQS_RAND_report_statistics(const unsigned long occurrences[256], const char *indent) {
	...
	if      (zscore == ZSCORE_BIGNEG) printf("less than -5.0 sigma ***\n");
