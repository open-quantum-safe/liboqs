 ...
 *
 * The file `src/kem/example_kem.c` contains two examples on using the OQS_KEM
 * API.
 *
 ...
 * length
 * member of the OQS_KEM object in question.  Since algorithms can be disabled
 * at
 * compile-time, the programmer should check that the OQS_KEM object is not
 * `NULL`.
 */
 ...
 * indicates either than an
 * invalid algorithm name was provided, or that the requested algorithm was
 ...
 * based
 * on the `length_*` members in this object or the per-scheme compile-time
 * macros
 * `OQS_KEM_*_length_*`.
 *
 * @param[in] kem The OQS_KEM object representing the KEM.
