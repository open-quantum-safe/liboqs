...
void PQCLEAN_HQC256_AVX2_hqc_pke_keygen(unsigned char *pk, unsigned char *sk) {
    ...
    PQCLEAN_HQC256_AVX2_vect_mul(s, y, h);
    PQCLEAN_HQC256_AVX2_vect_add(s, x, s, VEC_N_256_SIZE_64);

    // Parse keys to string
    PQCLEAN_HQC256_AVX2_hqc_public_key_to_string(pk, pk_seed, s);
    PQCLEAN_HQC256_AVX2_hqc_secret_key_to_string(sk, sk_seed, pk);
...
void PQCLEAN_HQC256_AVX2_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint64_t *m, unsigned char *theta, const unsigned char *pk) {
    ...
    PQCLEAN_HQC256_AVX2_hqc_public_key_from_string(h, s, pk);

    // Generate r1, r2 and e
    PQCLEAN_HQC256_AVX2_vect_set_random_fixed_weight(&seedexpander, r1, PARAM_OMEGA_R);
    PQCLEAN_HQC256_AVX2_vect_set_random_fixed_weight(&seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQC256_AVX2_vect_set_random_fixed_weight(&seedexpander, e, PARAM_OMEGA_E);

    // Compute u = r1 + r2.h
    PQCLEAN_HQC256_AVX2_vect_mul(u, r2, h);
    PQCLEAN_HQC256_AVX2_vect_add(u, r1, u, VEC_N_256_SIZE_64);

    // Compute v = m.G by encoding the message
    PQCLEAN_HQC256_AVX2_code_encode(v, m);
    PQCLEAN_HQC256_AVX2_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2);

    // Compute v = m.G + s.r2 + e
    PQCLEAN_HQC256_AVX2_vect_mul(tmp2, r2, s);
    PQCLEAN_HQC256_AVX2_vect_add(tmp2, e, tmp2, VEC_N_256_SIZE_64);
    PQCLEAN_HQC256_AVX2_vect_add(tmp2, tmp1, tmp2, VEC_N_256_SIZE_64);
    PQCLEAN_HQC256_AVX2_vect_resize(v, PARAM_N1N2, tmp2, PARAM_N);
...
void PQCLEAN_HQC256_AVX2_hqc_pke_decrypt(uint64_t *m, const uint64_t *u, const uint64_t *v, const unsigned char *sk) {
    ...
    PQCLEAN_HQC256_AVX2_hqc_secret_key_from_string(x, y, pk, sk);

    // Compute v - u.y
    PQCLEAN_HQC256_AVX2_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2);
    PQCLEAN_HQC256_AVX2_vect_mul(tmp2, y, u);
    PQCLEAN_HQC256_AVX2_vect_add(tmp2, tmp1, tmp2, VEC_N_256_SIZE_64);


    // Compute m by decoding v - u.y
    PQCLEAN_HQC256_AVX2_code_decode(m, tmp2);
}
