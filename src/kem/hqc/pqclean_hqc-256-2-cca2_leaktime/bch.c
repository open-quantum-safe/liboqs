 ...
 */
void PQCLEAN_HQC2562CCA2_LEAKTIME_bch_code_encode(uint8_t *codeword, const uint8_t *message) {
...
static size_t compute_elp(uint16_t *sigma, const uint16_t *syndromes) {
    ...
    for (size_t mu = 0; mu < PARAM_DELTA; ++mu) {
        ...
        uint16_t dd = PQCLEAN_HQC2562CCA2_LEAKTIME_gf_mul(d, PQCLEAN_HQC2562CCA2_LEAKTIME_gf_inverse(d_p)); // 0 if(d == 0)
        for (size_t i = 1; (i <= 2 * mu + 1) && (i <= PARAM_DELTA); ++i) {
            sigma[i] ^= PQCLEAN_HQC2562CCA2_LEAKTIME_gf_mul(dd, X_sigma_p[i]);
        }
        ...
        d = syndromes[2 * mu + 2];
        for (size_t i = 1; (i <= 2 * mu + 1) && (i <= PARAM_DELTA); ++i) {
            d ^= PQCLEAN_HQC2562CCA2_LEAKTIME_gf_mul(sigma[i], syndromes[2 * mu + 2 - i]);
        }
    }
...
static void compute_syndromes(uint16_t *syndromes, const uint8_t *vector) {
    ...
    PQCLEAN_HQC2562CCA2_LEAKTIME_fft_t_preprocess_bch_codeword(w, vector);
    PQCLEAN_HQC2562CCA2_LEAKTIME_fft_t(syndromes, w, 2 * PARAM_DELTA);
}
...
static void compute_roots(uint8_t *error, const uint16_t *sigma) {
    ...
    PQCLEAN_HQC2562CCA2_LEAKTIME_fft(w, sigma, PARAM_DELTA + 1);
    PQCLEAN_HQC2562CCA2_LEAKTIME_fft_retrieve_bch_error_poly(error, w);
}
 ...
 * @param[in] vector Array of size VEC_N1_SIZE_BYTES storing the received word
 */
void PQCLEAN_HQC2562CCA2_LEAKTIME_bch_code_decode(uint8_t *message, uint8_t *vector) {
    ...
    PQCLEAN_HQC2562CCA2_LEAKTIME_vect_add(vector, vector, error, VEC_N1_SIZE_BYTES);
