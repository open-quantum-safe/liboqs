...
static uint16_t compute_elp(uint16_t *sigma, const uint16_t *syndromes) {
    ...
    for (mu = 0; (mu < (2 * PARAM_DELTA)); ++mu) {
        ...
        uint16_t not_last = (uint16_t)(((uint16_t)(mu ^ (2 * PARAM_DELTA - 1))) | 
                            (uint16_t)(~(mu ^ (2 * PARAM_DELTA - 1)) + 1));
        not_last = (uint16_t)(-(not_last >> 15 | (uint16_t)(-not_last) >> 15));
        /* Simplified: not_last = 0xFFFF when mu != last, 0x0000 when mu == last */
        not_last = (uint16_t)(-((uint16_t)(mu - (2 * PARAM_DELTA - 1)) != 0));
        ...
        for (i = 1; (i <= mu + 1) && (i <= PARAM_DELTA); ++i) {
            d ^= not_last & PQCLEAN_HQC192_CLEAN_gf_mul(sigma[i], syndromes[mu + 1 - i]);
        }
    }
