 ...
 */
 void PQCLEAN_HQC192_CLEAN_hqc_pke_keygen_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed) {
    ...
    // Create seed_expanders for public key and secret key
    PQCLEAN_HQC192_CLEAN_seedexpander(&seed_seedexpander, sk_seed, SEED_BYTES);
    PQCLEAN_HQC192_CLEAN_seedexpander_init(&sk_seedexpander, sk_seed, SEED_BYTES);

    PQCLEAN_HQC192_CLEAN_seedexpander(&seed_seedexpander, pk_seed, SEED_BYTES);
    PQCLEAN_HQC192_CLEAN_seedexpander_init(&pk_seedexpander, pk_seed, SEED_BYTES);

    // Derive sigma from seed for deterministic key generation
    PQCLEAN_HQC192_CLEAN_seedexpander(&seed_seedexpander, sigma, VEC_K_SIZE_BYTES);
