 ...
 * @return an integer equal to a % (HQC256_PARAM_N - i)
 */
static inline uint16_t HQC256_barrett_reduce(uint32_t a, uint16_t i) {
    uint32_t t;

    t = ((HQC256_v_val[i] * a + HQC256_v_val[i]) >> HQC256_SHBIT32);
    t *= (HQC256_PARAM_N - i);
    return (uint16_t)(a - t);
}
...
void HQC256_vect_set_random_fixed_weight(seedexpander_state *ctx, __m256i *v256, uint16_t weight) {
    uint32_t rand_u32[HQC256_PARAM_OMEGA_R] = {0};
    uint32_t tmp[HQC256_PARAM_OMEGA_R] = {0};
    __m256i bit256[HQC256_PARAM_OMEGA_R];
    __m256i bloc256[HQC256_PARAM_OMEGA_R];
    static __m256i posCmp256 = (__m256i){0UL,1UL,2UL,3UL};

    HQC256_seedexpander(ctx, (uint8_t *)&rand_u32, 4 * weight);

    for (size_t i = 0; i < weight; ++i) {
        tmp[i] = i + HQC256_barrett_reduce(rand_u32[i], i);
    }
    ...
    for (int32_t i = (weight - 1); i -- > 0;) {
        ...
        for (size_t j = i + 1; j < weight; ++j) {
            found |= HQC256_compare_u32(tmp[j], tmp[i]);
        }
...
/**
 * @brief Generates a random vector of dimension <b>HQC256_PARAM_N</b>
 *
 * This function generates a random binary vector of dimension <b>HQC256_PARAM_N</b>. It generates a random
...
void HQC256_vect_set_random(seedexpander_state *ctx, uint64_t *v) {
    uint8_t rand_bytes[HQC256_VEC_N_SIZE_BYTES] = {0};

    HQC256_seedexpander(ctx, rand_bytes, HQC256_VEC_N_SIZE_BYTES);

    memcpy(v, rand_bytes, HQC256_VEC_N_SIZE_BYTES);
    v[HQC256_VEC_N_SIZE_64 - 1] &= HQC256_BITMASK(HQC256_PARAM_N, 64);
}
...
void HQC256_vect_set_random_from_prng(uint64_t *v, uint32_t size_v) {
    ...
    HQC256_shake_prng(rand_bytes, size_v << 3);
...
void HQC256_vect_resize(uint64_t *o, uint32_t size_o, const uint64_t *v, uint32_t size_v) {
    ...
    if (size_o < size_v) {
        ...
        memcpy(o, v, HQC256_VEC_N1N2_SIZE_BYTES);

        for (int8_t i = 0 ; i < val ; ++i) {
            o[HQC256_VEC_N1N2_SIZE_64 - 1] &= (mask >> i);
        }
    } else {
        memcpy(o, v, HQC256_CEIL_DIVIDE(size_v, 8));
    }
}
...
void HQC256_vect_print(const uint64_t *v, const uint32_t size) {
    if(size == HQC256_VEC_K_SIZE_BYTES) {
        uint8_t tmp [HQC256_VEC_K_SIZE_BYTES] = {0};
        memcpy(tmp, v, HQC256_VEC_K_SIZE_BYTES);
        for (uint32_t i = 0; i < HQC256_VEC_K_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    } else if (size == HQC256_VEC_N_SIZE_BYTES) {
        uint8_t tmp [HQC256_VEC_N_SIZE_BYTES] = {0};
        memcpy(tmp, v, HQC256_VEC_N_SIZE_BYTES);
        for (uint32_t i = 0; i < HQC256_VEC_N_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    } else if (size == HQC256_VEC_N1N2_SIZE_BYTES) {
        uint8_t tmp [HQC256_VEC_N1N2_SIZE_BYTES] = {0};
        memcpy(tmp, v, HQC256_VEC_N1N2_SIZE_BYTES);
        for (uint32_t i = 0; i < HQC256_VEC_N1N2_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    }  else if (size == HQC256_VEC_N1_SIZE_BYTES) {
        uint8_t tmp [HQC256_VEC_N1_SIZE_BYTES] = {0};
        memcpy(tmp, v, HQC256_VEC_N1_SIZE_BYTES);
        for (uint32_t i = 0; i < HQC256_VEC_N1_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    }
}
