...
#define HQC192_VEC_N_ARRAY_SIZE_VEC                                            \
    HQC192_CEIL_DIVIDE(                                                        \
        HQC192_PARAM_N_MULT,                                                   \
        256) /*!< The number of needed vectors to store PARAM_N bits*/
#define WORD 64
#define LAST64 (HQC192_PARAM_N >> 6)
...
#define T_TM3R_3W (HQC192_PARAM_N_MULT / 3)
#define T_TM3R (HQC192_PARAM_N_MULT + 384)
#define T_TM3R_3W_256 ((T_TM3R_3W + 128) / (4 * WORD))
#define T_TM3R_3W_64 (T_TM3R_3W_256 << 2)

__m256i HQC192_a1_times_a2[HQC192_VEC_N_256_SIZE_64 >> 1];
__m256i HQC192_o256[HQC192_VEC_N_ARRAY_SIZE_VEC];
uint64_t *HQC192_tmp_reduce = (uint64_t *) HQC192_o256;

uint64_t HQC192_bloc64[HQC192_PARAM_OMEGA_R]; // Allocation with the biggest possible weight
uint64_t HQC192_bit64[HQC192_PARAM_OMEGA_R]; // Allocation with the biggest possible weight
...
static inline void reduce(__m256i *o, const __m256i *a256) {
    ...
    static const int32_t dec64 = HQC192_PARAM_N & 0x3f;
    static int32_t d0;
    int32_t i, i2;

    d0 = WORD - dec64;
    for (i = LAST64; i < (HQC192_PARAM_N >> 5) - 4; i += 4) {
    ...
    for (; i < LAST64 + 1; i++) {
        ...
        r ^= carry;
        HQC192_tmp_reduce[i] = a[i] ^ r;
    }

    HQC192_tmp_reduce[LAST64] &= HQC192_RED_MASK;
    memcpy(o, HQC192_tmp_reduce, HQC192_VEC_N_SIZE_BYTES);
}
...
void toom_3_mult(__m256i *Out, const __m256i *A256, const __m256i *B256) {
    ...
    U1_64 = ((uint64_t *)W4);
    tmp[0] =
        W2[0] ^ W3[0] ^ W4[0] ^ (__m256i) { 0x0ul, 0x0ul, 0x0ul, U1_64[0] };
    U1_64++;
    ...
    for (int32_t i = 0; i < (T_TM3R_3W_256 << 1) - 1; i++) {
        int32_t i4 = i << 2;
        tmp[i] = _mm256_lddqu_si256((__m256i const *)(&U1_64[i4])) ^
 ...
 */
void HQC192_vect_mul(__m256i *o, const __m256i *a1, const __m256i *a2) {
    toom_3_mult(HQC192_a1_times_a2, a1, a2);
    reduce(o, HQC192_a1_times_a2);

// clear all
#ifdef __STDC_LIB_EXT1__
    memset_s(HQC192_a1_times_a2, 0, (HQC192_VEC_N_SIZE_64 >> 1) * sizeof(__m256i));
#else
    memset(HQC192_a1_times_a2, 0, (HQC192_VEC_N_SIZE_64 >> 1) * sizeof(__m256i));
#endif
}
