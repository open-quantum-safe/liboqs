...
static void HQC192_compute_fft_betas(uint16_t *betas);
static void HQC192_compute_subset_sums(uint16_t *subset_sums, const uint16_t *set, uint16_t set_size);
static void HQC192_radix(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f);
static void HQC192_radix_big(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f);
static void HQC192_fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs, uint8_t m, uint32_t m_f, const uint16_t *betas);
...
static void HQC192_radix(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f) {
    switch (m_f) {
        ...
        default:
            HQC192_radix_big(f0, f1, f, m_f);
            break;
    }
}

static void HQC192_radix_big(uint16_t *f0, uint16_t *f1, const uint16_t *f, uint32_t m_f) {
    uint16_t Q[2 * (1 << (HQC192_PARAM_FFT - 2)) + 1] = {0};
    uint16_t R[2 * (1 << (HQC192_PARAM_FFT - 2)) + 1] = {0};

    uint16_t Q0[1 << (HQC192_PARAM_FFT - 2)] = {0};
    uint16_t Q1[1 << (HQC192_PARAM_FFT - 2)] = {0};
    uint16_t R0[1 << (HQC192_PARAM_FFT - 2)] = {0};
    uint16_t R1[1 << (HQC192_PARAM_FFT - 2)] = {0};
 ...
 */
static void HQC192_fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs, uint8_t m, uint32_t m_f, const uint16_t *betas) {
    uint16_t f0[1 << (HQC192_PARAM_FFT - 2)] = {0};
    uint16_t f1[1 << (HQC192_PARAM_FFT - 2)] = {0};
    uint16_t gammas[HQC192_PARAM_M - 2] = {0};
    uint16_t deltas[HQC192_PARAM_M - 2] = {0};
    uint16_t gammas_sums[1 << (HQC192_PARAM_M - 2)] = {0};
    uint16_t u[1 << (HQC192_PARAM_M - 2)] = {0};
    uint16_t v[1 << (HQC192_PARAM_M - 2)] = {0};
    uint16_t tmp[HQC192_PARAM_M - (HQC192_PARAM_FFT - 1)] = {0};
    ...
    if (m_f == 1) {
        for (i = 0; i < m; ++i) {
            tmp[i] = HQC192_gf_mul(betas[i], f[1]);
        }
    ...
    for (i = 0; i + 1 < m; ++i) {
        gammas[i] = HQC192_gf_mul(betas[i], HQC192_gf_inverse(betas[m - 1]));
        deltas[i] = HQC192_gf_square(gammas[i]) ^ gammas[i];
    }

    // Compute gammas sums
    HQC192_compute_subset_sums(gammas_sums, gammas, m - 1);

    // Step 5
    HQC192_fft_rec(u, f0, (f_coeffs + 1) / 2, m - 1, m_f - 1, deltas);
    ...
    if (f_coeffs <= 3) { // 3-coefficient polynomial f case: f1 is constant
        ...
        HQC192_fft_rec(v, f1, f_coeffs / 2, m - 1, m_f - 1, deltas);
...
void HQC192_fft(uint16_t *w, const uint16_t *f, size_t f_coeffs) {
    uint16_t betas[HQC192_PARAM_M - 1] = {0};
    uint16_t betas_sums[1 << (HQC192_PARAM_M - 1)] = {0};
    uint16_t f0[1 << (HQC192_PARAM_FFT - 1)] = {0};
    uint16_t f1[1 << (HQC192_PARAM_FFT - 1)] = {0};
    uint16_t deltas[HQC192_PARAM_M - 1] = {0};
    uint16_t u[1 << (HQC192_PARAM_M - 1)] = {0};
    uint16_t v[1 << (HQC192_PARAM_M - 1)] = {0};
    ...
    // Compute gammas sums
    HQC192_compute_subset_sums(betas_sums, betas, HQC192_PARAM_M - 1);
    ...
    // Step 5
    HQC192_fft_rec(u, f0, (f_coeffs + 1) / 2, HQC192_PARAM_M - 1, HQC192_PARAM_FFT - 1, deltas);
    HQC192_fft_rec(v, f1, f_coeffs / 2, HQC192_PARAM_M - 1, HQC192_PARAM_FFT - 1, deltas);

    k = 1 << (HQC192_PARAM_M - 1);
...
void HQC192_fft_retrieve_error_poly(uint8_t *error, const uint16_t *w) {
    uint16_t gammas[HQC192_PARAM_M - 1] = {0};
    uint16_t gammas_sums[1 << (HQC192_PARAM_M - 1)] = {0};
    uint16_t k;
    size_t i, index;

    HQC192_compute_fft_betas(gammas);
    HQC192_compute_subset_sums(gammas_sums, gammas, HQC192_PARAM_M - 1);
    ...
    for (i = 1; i < k; ++i) {
        index = HQC192_PARAM_GF_MUL_ORDER - HQC192_gf_log[gammas_sums[i]];
        error[index] ^= 1 ^ ((uint16_t) - w[i] >> 15);

        index = HQC192_PARAM_GF_MUL_ORDER - HQC192_gf_log[gammas_sums[i] ^ 1];
