...
static uint16_t HQC192_gf_reduce(uint64_t x, size_t deg_x);
 ...
 */
void HQC192_gf_generate(uint16_t *exp, uint16_t *log, const int16_t m) {
    uint16_t elt = 1;
    uint16_t alpha = 2; // primitive element of GF(2^PARAM_M)
    uint16_t gf_poly = HQC192_PARAM_GF_POLY;
...
static uint16_t HQC192_gf_reduce(uint64_t x, size_t deg_x) {
    ...
    steps = HQC192_CEIL_DIVIDE(deg_x - (HQC192_PARAM_M - 1), HQC192_PARAM_GF_POLY_M2);

    // Reduce
    for (i = 0; i < steps; ++i) {
        mod = x >> HQC192_PARAM_M;
        x &= (1 << HQC192_PARAM_M) - 1;
        x ^= mod;

        z1 = 0;
        rmdr = HQC192_PARAM_GF_POLY ^ 1;
        for (j = HQC192_PARAM_GF_POLY_WT - 2; j; --j) {
            z2 = __tzcnt_u16(rmdr);
            dist = (uint16_t) (z2 - z1);
            mod <<= dist;
            x ^= mod;
 ...
 */
uint16_t HQC192_gf_mul(uint16_t a, uint16_t b) {
    ...
    return HQC192_gf_reduce(ab, 2*(HQC192_PARAM_M-1));
}
 ...
 * 
 */
__m256i HQC192_gf_mul_vect(__m256i a, __m256i b) {
    ...
    __m128i abl0 = _mm_clmulepi64_si128(al & HQC192_maskl, bl & HQC192_maskl, 0x0);
    abl0 &= HQC192_middlemaskl;
    abl0 ^= (_mm_clmulepi64_si128(al & HQC192_maskh, bl & HQC192_maskh, 0x0) & HQC192_middlemaskh);

    __m128i abh0 = _mm_clmulepi64_si128(al & HQC192_maskl, bl & HQC192_maskl, 0x11);
    abh0 &= HQC192_middlemaskl;
    abh0 ^= (_mm_clmulepi64_si128(al & HQC192_maskh, bl & HQC192_maskh, 0x11) & HQC192_middlemaskh);

    abl0 = _mm_shuffle_epi8(abl0, HQC192_indexl);
    abl0 ^= _mm_shuffle_epi8(abh0, HQC192_indexh);

    __m128i abl1 = _mm_clmulepi64_si128(ah & HQC192_maskl, bh & HQC192_maskl, 0x0);
    abl1 &= HQC192_middlemaskl;
    abl1 ^= (_mm_clmulepi64_si128(ah & HQC192_maskh, bh & HQC192_maskh, 0x0) & HQC192_middlemaskh);

    __m128i abh1 = _mm_clmulepi64_si128(ah & HQC192_maskl, bh & HQC192_maskl, 0x11);
    abh1 &= HQC192_middlemaskl;
    abh1 ^= (_mm_clmulepi64_si128(ah & HQC192_maskh, bh & HQC192_maskh, 0x11) & HQC192_middlemaskh);

    abl1 = _mm_shuffle_epi8(abl1, HQC192_indexl);
    abl1 ^= _mm_shuffle_epi8(abh1, HQC192_indexh);

    __m256i ret = _mm256_set_m128i(abl1, abl0);

    __m256i aux = HQC192_mr0;

    for (int32_t i = 0; i < 7; i++) {
        ret ^= HQC192_red[i] & _mm256_cmpeq_epi16((ret & aux), aux);
        aux = aux << 1;
    }
 ...
 * @param[in] a Element of GF(2^GF_M)
 */
uint16_t HQC192_gf_square(uint16_t a) {
    uint32_t b = a;
    uint32_t s = b & 1;     
    for (size_t i = 1; i < HQC192_PARAM_M; ++i) {
        b <<= 1;
        s ^= b & (1 << 2 * i);
    }
 ...
 * @param[in] a Element of GF(2^GF_M)
 */
uint16_t HQC192_gf_inverse(uint16_t a) {
    uint16_t inv = a;
    uint16_t tmp1, tmp2;
 ...
 */
uint16_t HQC192_gf_mod(uint16_t i) {
    uint16_t tmp = i - HQC192_PARAM_GF_MUL_ORDER;

    // mask = 0xffff if (i < GF_MUL_ORDER)
    int16_t mask = -(tmp >> 15);

    return tmp + (mask & HQC192_PARAM_GF_MUL_ORDER);
}
