...
int PQCLEAN_HQC2563CCA2_LEAKTIME_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    PQCLEAN_HQC2563CCA2_LEAKTIME_hqc_pke_keygen(pk, sk);
    return 0;
}
...
int PQCLEAN_HQC2563CCA2_LEAKTIME_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
    ...
    // Computing m
    PQCLEAN_HQC2563CCA2_LEAKTIME_vect_set_random_from_randombytes(m);
    ...
    // Encrypting m
    PQCLEAN_HQC2563CCA2_LEAKTIME_hqc_pke_encrypt(u, v, m, theta, pk);
    ...
    PQCLEAN_HQC2563CCA2_LEAKTIME_hqc_ciphertext_to_string(ct, u, v, d);
...
int PQCLEAN_HQC2563CCA2_LEAKTIME_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
    ...
    PQCLEAN_HQC2563CCA2_LEAKTIME_hqc_ciphertext_from_string(u, v, d, ct);

    // Retrieving pk from sk
    memcpy(pk, sk + SEED_BYTES, PUBLIC_KEY_BYTES);

    // Decryting
    PQCLEAN_HQC2563CCA2_LEAKTIME_hqc_pke_decrypt(m, u, v, sk);
    ...
    // Encrypting m'
    PQCLEAN_HQC2563CCA2_LEAKTIME_hqc_pke_encrypt(u2, v2, m, theta, pk);

    // Checking that c = c' and abort otherwise
    if (PQCLEAN_HQC2563CCA2_LEAKTIME_vect_compare(u, u2, VEC_N_SIZE_BYTES) != 0 ||
            PQCLEAN_HQC2563CCA2_LEAKTIME_vect_compare(v, v2, VEC_N1N2_SIZE_BYTES) != 0) {
        abort = 1;
    }
