...
static void fft_t_rec(uint16_t *f, const uint16_t *w, size_t f_coeffs,
    ...
    if (m_f == 1) {
        ...
        for (size_t j = 0; j < m; ++j) {
            for (size_t ki = 0; ki < (((size_t)1) << j); ++ki) {
                ...
                f[1] ^= PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(betas_sums[index], w[index]);
            }
        }
    ...
    for (uint8_t i = 0; i < m - 1; ++i) {
        gammas[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(betas[i], PQCLEAN_HQC2563CCA2_LEAKTIME_gf_inverse(betas[m - 1]));
        deltas[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_square(gammas[i]) ^ gammas[i];
    }
    ...
    if (f_coeffs <= 3) { // 3-coefficient polynomial f case
        ...
        for (size_t i = 1; i < k; ++i) {
            u[i] = w[i] ^ w[k + i];
            f1[0] ^= PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(gammas_sums[i], u[i]) ^ w[k + i];
        }
        ...
        for (size_t i = 1; i < k; ++i) {
            u[i] = w[i] ^ w[k + i];
            v[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(gammas_sums[i], u[i]) ^ w[k + i];
        }
    ...
    if (betas[m - 1] != 1) {
        ...
        for (size_t i = 1; i < (((size_t)1) << m_f); ++i) {
            beta_m_pow = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(beta_m_pow, betas[m - 1]);
            f[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(beta_m_pow, f[i]);
        }
    }
}
 ...
 */
void PQCLEAN_HQC2563CCA2_LEAKTIME_fft_t(uint16_t *f, const uint16_t *w, size_t f_coeffs) {
    ...
    for (size_t i = 1; i < k; ++i) {
        u[i] = w[i] ^ w[k + i];
        v[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(betas_sums[i], u[i]) ^ w[k + i];
    }

    // Compute deltas
    for (size_t i = 0; i < PARAM_M - 1; ++i) {
        deltas[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_square(betas[i]) ^ betas[i];
    }
...
static void fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs,
    ...
    if (m_f == 1) {
        ...
        for (size_t i = 0; i < m; ++i) {
            tmp[i] = PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(betas[i], f[1]);
        }
    ...
    if (f_coeffs <= 3) { // 3-coefficient polynomial f case: f1 is constant
        ...
        w[k] = u[0] ^ f1[0];
        for (size_t i = 1; i < k; ++i) {
            w[i] = u[i] ^ PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(gammas_sums[i], f1[0]);
            w[k + i] = w[i] ^ f1[0];
        }
    } else {
        ...
        w[k] ^= u[0];
        for (size_t i = 1; i < k; ++i) {
            w[i] = u[i] ^ PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(gammas_sums[i], v[i]);
            w[k + i] ^= w[i];
        }
    }
}
 ...
 */
void PQCLEAN_HQC2563CCA2_LEAKTIME_fft(uint16_t *w, const uint16_t *f, size_t f_coeffs) {
    ...
    // Find other roots
    for (size_t i = 1; i < k; ++i) {
        w[i] = u[i] ^ PQCLEAN_HQC2563CCA2_LEAKTIME_gf_mul(betas_sums[i], v[i]);
        w[k + i] ^= w[i];
    }
}
 ...
 */
void PQCLEAN_HQC2563CCA2_LEAKTIME_fft_t_preprocess_bch_codeword(uint16_t *w, const uint8_t *vector) {
    ...
    w[k] = -r[0] & 1;
    for (i = 1; i < k; ++i) {
        w[i] = -r[PQCLEAN_HQC2563CCA2_LEAKTIME_gf_log(gammas_sums[i])] & gammas_sums[i];
        w[k + i] = -r[PQCLEAN_HQC2563CCA2_LEAKTIME_gf_log(gammas_sums[i] ^ 1)] & (gammas_sums[i] ^ 1);
    }
}
 ...
 */
void PQCLEAN_HQC2563CCA2_LEAKTIME_fft_retrieve_bch_error_poly(uint8_t *error, const uint16_t *w) {
    uint16_t gammas[PARAM_M - 1];
    uint16_t gammas_sums[1 << (PARAM_M - 1)];
    size_t k = 1 << (PARAM_M - 1);
    ...
    for (size_t i = 1; i < k; ++i) {
        index = PARAM_GF_MUL_ORDER - PQCLEAN_HQC2563CCA2_LEAKTIME_gf_log(gammas_sums[i]);
        bit = 1 ^ ((uint16_t) - w[i] >> 15);
        error[index / 8] ^= bit << (index % 8);

        index = PARAM_GF_MUL_ORDER - PQCLEAN_HQC2563CCA2_LEAKTIME_gf_log(gammas_sums[i] ^ 1);
