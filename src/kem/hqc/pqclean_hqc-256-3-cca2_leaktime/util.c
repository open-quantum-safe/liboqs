...
void PQCLEAN_HQC2563CCA2_LEAKTIME_store8(unsigned char *out, uint64_t in) {
    out[0] = (in >> 0x00) & 0xFF;
    out[1] = (in >> 0x08) & 0xFF;
    out[2] = (in >> 0x10) & 0xFF;
    out[3] = (in >> 0x18) & 0xFF;
...
uint64_t PQCLEAN_HQC2563CCA2_LEAKTIME_load8(const unsigned char *in) {
    uint64_t ret = in[7];
...
void PQCLEAN_HQC2563CCA2_LEAKTIME_load8_arr(uint64_t *out64, size_t outlen, const uint8_t *in8, size_t inlen) {
    size_t index_in = 0;
    size_t index_out = 0;
    ...
    if (inlen >= 8 && outlen >= 1) {
        while (index_out < outlen && index_in + 8 <= inlen) {
            out64[index_out] = PQCLEAN_HQC2563CCA2_LEAKTIME_load8(in8 + index_in);
...
void PQCLEAN_HQC2563CCA2_LEAKTIME_store8_arr(uint8_t *out8, size_t outlen, const uint64_t *in64, size_t inlen) {
