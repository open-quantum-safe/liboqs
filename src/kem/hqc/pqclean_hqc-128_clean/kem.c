 ...
 * @returns 0 if keygen is successful
 */
 int PQCLEAN_HQC128_CLEAN_crypto_kem_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed) {
    PQCLEAN_HQC128_CLEAN_hqc_pke_keygen_derand(pk, sk, seed);
    return 0;
}
 ...
 * @returns 0 if encapsulation is successful
 */
 int PQCLEAN_HQC128_CLEAN_crypto_kem_enc_derand(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *seed) {
    ...
    // Computing m
    PQCLEAN_HQC128_CLEAN_seedexpander(&seed_seedexpander, m, VEC_K_SIZE_BYTES);

    // Computing theta
    PQCLEAN_HQC128_CLEAN_seedexpander(&seed_seedexpander, salt, SALT_SIZE_BYTES);
