...
#include "scalars_2x.inc"
    ...
    for (j = 0; j <= 5; j++) {
        ...
        if (j < 5) {
            PQCLEAN_MCELIECE460896F_VEC_vec_mul(in[0], in[0], s[j][0]);
            PQCLEAN_MCELIECE460896F_VEC_vec_mul(in[1], in[1], s[j][1]);
        }
    }
}
...
#include "consts.inc"
    ...
    for (i = 0; i < 7; i++) {
        ...
        PQCLEAN_MCELIECE460896F_VEC_vec_mul(pre[i], in[1], pre[i]);
    }
    ...
    for (i = 0; i < GFBITS; i++) {
        ...
        PQCLEAN_MCELIECE460896F_VEC_transpose_64x64(buf + 64, buf + 64);
    ...
    for (i = 1; i <= 6; i++) {
        ...
        for (j = 0; j < 128; j += 2 * s) {
            for (k = j; k < j + s; k++) {
                PQCLEAN_MCELIECE460896F_VEC_vec_mul(tmp, out[k + s], consts[ consts_ptr + (k - j) ]);
...
void PQCLEAN_MCELIECE460896F_VEC_fft(vec out[][GFBITS], vec in[][GFBITS]) {
