...
int PQCLEAN_MCELIECE348864F_VEC_pk_gen(uint8_t *pk, uint32_t *perm, const uint8_t *sk) {
...
#define NBLOCKS_I  ((GFBITS * SYS_T + 63) / 64)
    ...
    PQCLEAN_MCELIECE348864F_VEC_vec_copy(prod[0], eval[0]);

    for (i = 1; i < 64; i++) {
        PQCLEAN_MCELIECE348864F_VEC_vec_mul(prod[i], prod[i - 1], eval[i]);
    }

    PQCLEAN_MCELIECE348864F_VEC_vec_inv(tmp, prod[63]);

    for (i = 62; i >= 0; i--) {
        PQCLEAN_MCELIECE348864F_VEC_vec_mul(prod[i + 1], prod[i], tmp);
        PQCLEAN_MCELIECE348864F_VEC_vec_mul(tmp, tmp, eval[i + 1]);
    }

    PQCLEAN_MCELIECE348864F_VEC_vec_copy(prod[0], tmp);
    ...
    for (i = 1; i < SYS_T; i++) {
        for (j = 0; j < NBLOCKS_H; j++) {
            PQCLEAN_MCELIECE348864F_VEC_vec_mul(prod[j], prod[j], consts[j]);
    ...
    for (i = 0; i < GFBITS * SYS_T; i++) {
        for (j = NBLOCKS_I; j < NBLOCKS_H - 1; j++) {
            PQCLEAN_MCELIECE348864F_VEC_store8(pk, mat[i][j]);
            pk += 8;
        }

        PQCLEAN_MCELIECE348864F_VEC_store_i(pk, mat[i][j], PK_ROW_BYTES % 8);
