...
/* check if a == 0 */
gf PQCLEAN_MCELIECE460896_VEC_gf_iszero(gf a) {
    uint32_t t = a;
...
gf PQCLEAN_MCELIECE460896_VEC_gf_mul(gf in0, gf in1) {
    int i;
...
/* return num/den */
gf PQCLEAN_MCELIECE460896_VEC_gf_frac(gf den, gf num) {
    gf tmp_11;
    gf tmp_1111;
    gf out;
...
/* return 1/den */
gf PQCLEAN_MCELIECE460896_VEC_gf_inv(gf in) {
    return PQCLEAN_MCELIECE460896_VEC_gf_frac(in, ((gf) 1));
}

/* multiplication in GF((2^m)^t) */
void PQCLEAN_MCELIECE460896_VEC_GF_mul(gf *out, const gf *in0, const gf *in1) {
    int i, j;
    ...
    for (i = 0; i < 96; i++) {
        for (j = 0; j < 96; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE460896_VEC_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = 190; i >= 96; i--) {
        prod[i - 85] ^= PQCLEAN_MCELIECE460896_VEC_gf_mul(prod[i], (gf) 714);
        prod[i - 91] ^= PQCLEAN_MCELIECE460896_VEC_gf_mul(prod[i], (gf) 5296);
        prod[i - 92] ^= PQCLEAN_MCELIECE460896_VEC_gf_mul(prod[i], (gf) 728);
        prod[i - 96] ^= PQCLEAN_MCELIECE460896_VEC_gf_mul(prod[i], (gf) 5881);
    }
