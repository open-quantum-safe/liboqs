...
#include "scalars_4x.inc"
    ...
    for (j = 6; j >= 0; j--) {
        if (j < 6) {
            PQCLEAN_MCELIECE6688128F_VEC_vec_mul(in[0], in[0], s[j][0]); // scaling
            PQCLEAN_MCELIECE6688128F_VEC_vec_mul(in[1], in[1], s[j][1]); // scaling
            PQCLEAN_MCELIECE6688128F_VEC_vec_mul(in[2], in[2], s[j][2]); // scaling
            PQCLEAN_MCELIECE6688128F_VEC_vec_mul(in[3], in[3], s[j][3]); // scaling
        }
...
#include "consts.inc"
    ...
    for (i = 6; i >= 0; i--) {
        ...
        for (j = 0; j < 128; j += 2 * s) {
            for (k = j; k < j + s; k++) {
                ...
                PQCLEAN_MCELIECE6688128F_VEC_vec_mul(tmp, in[k], consts[ consts_ptr + (k - j) ]);
    ...
    for (i = 0; i < GFBITS; i++) {
        ...
        PQCLEAN_MCELIECE6688128F_VEC_transpose_64x64(buf[1], buf[1]);
    ...
    for (j = 0; j < GFBITS; j++) {
        tmp[j] = PQCLEAN_MCELIECE6688128F_VEC_vec_setbits((beta[0] >> j) & 1);
    }

    PQCLEAN_MCELIECE6688128F_VEC_vec_mul(out[2], pre[0][0], tmp);
    PQCLEAN_MCELIECE6688128F_VEC_vec_mul(out[3], pre[0][1], tmp);

    for (i = 1; i < 6; i++) {
        for (j = 0; j < GFBITS; j++) {
            tmp[j] = PQCLEAN_MCELIECE6688128F_VEC_vec_setbits((beta[i] >> j) & 1);
        }

        PQCLEAN_MCELIECE6688128F_VEC_vec_mul(pre[i][0], pre[i][0], tmp);
        PQCLEAN_MCELIECE6688128F_VEC_vec_mul(pre[i][1], pre[i][1], tmp);
...
void PQCLEAN_MCELIECE6688128F_VEC_fft_tr(vec out[][GFBITS], vec in[][ GFBITS ]) {
