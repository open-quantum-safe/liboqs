...
gf PQCLEAN_MCELIECE6688128F_VEC_gf_inv(gf in) {
    return PQCLEAN_MCELIECE6688128F_VEC_gf_frac(in, ((gf) 1));
}
...
void PQCLEAN_MCELIECE6688128F_VEC_GF_mul(gf *out, const gf *in0, const gf *in1) {
    ...
    for (i = 0; i < 128; i++) {
        for (j = 0; j < 128; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE6688128F_VEC_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = 254; i >= 128; i--) {
        prod[i - 123] ^= PQCLEAN_MCELIECE6688128F_VEC_gf_mul(prod[i], (gf) 7682);
        prod[i - 125] ^= PQCLEAN_MCELIECE6688128F_VEC_gf_mul(prod[i], (gf) 2159);
        prod[i - 128] ^= PQCLEAN_MCELIECE6688128F_VEC_gf_mul(prod[i], (gf) 3597);
    }
