...
OQS_KEM *OQS_KEM_classic_mceliece_460896f_new(void) {
	...
	kem->length_encaps_seed = OQS_KEM_classic_mceliece_460896f_length_encaps_seed;
...
extern int PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed);
extern int PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk);
extern int PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_enc_derand(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *seed);
...
extern int PQCLEAN_MCELIECE460896F_AVX2_crypto_kem_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed);
extern int PQCLEAN_MCELIECE460896F_AVX2_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk);
extern int PQCLEAN_MCELIECE460896F_AVX2_crypto_kem_enc_derand(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *seed);
...
OQS_API OQS_STATUS OQS_KEM_classic_mceliece_460896f_encaps_derand(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *seed) {
		...
		if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI1)) {
	#endif /* OQS_DIST_BUILD */
			return (OQS_STATUS) PQCLEAN_MCELIECE460896F_AVX2_crypto_kem_enc_derand(ciphertext, shared_secret, public_key, seed);
	#if defined(OQS_DIST_BUILD)
		} else {
			return (OQS_STATUS) PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_enc_derand(ciphertext, shared_secret, public_key, seed);
		}
	#endif /* OQS_DIST_BUILD */
	#else
		return (OQS_STATUS) PQCLEAN_MCELIECE460896F_CLEAN_crypto_kem_enc_derand(ciphertext, shared_secret, public_key, seed);
