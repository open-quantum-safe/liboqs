...
gf PQCLEAN_MCELIECE6960119F_AVX_gf_inv(gf in) {
    return PQCLEAN_MCELIECE6960119F_AVX_gf_frac(in, ((gf) 1));
}
...
void PQCLEAN_MCELIECE6960119F_AVX_GF_mul(gf *out, const gf *in0, const gf *in1) {
    ...
    for (i = 0; i < 119; i++) {
        for (j = 0; j < 119; j++) {
            prod[i + j] ^= PQCLEAN_MCELIECE6960119F_AVX_gf_mul(in0[i], in1[j]);
        }
    }
    ...
    for (i = 236; i >= 119; i--) {
        prod[i - 117] ^= PQCLEAN_MCELIECE6960119F_AVX_gf_mul(prod[i], (gf) 6400);
        prod[i - 119] ^= PQCLEAN_MCELIECE6960119F_AVX_gf_mul(prod[i], (gf) 3134);
    }
