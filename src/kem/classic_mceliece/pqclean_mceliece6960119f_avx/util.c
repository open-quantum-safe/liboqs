...
void PQCLEAN_MCELIECE6960119F_AVX_irr_load(vec128 *out, const unsigned char *in) {
    ...
    for (i = 0; i < SYS_T; i++) {
        irr[i] = PQCLEAN_MCELIECE6960119F_AVX_load2(in + i * 2);
        irr[i] &= GFMASK;
    }
    ...
    for (i = 0; i < GFBITS; i++) {
        ...
        out[i] = PQCLEAN_MCELIECE6960119F_AVX_vec128_set2x(v[0], v[1]);
    }
}
...
vec128 PQCLEAN_MCELIECE6960119F_AVX_load16(const unsigned char *in) {
    return PQCLEAN_MCELIECE6960119F_AVX_vec128_set2x( PQCLEAN_MCELIECE6960119F_AVX_load8(in), PQCLEAN_MCELIECE6960119F_AVX_load8(in + 8) );
}

void PQCLEAN_MCELIECE6960119F_AVX_store16(unsigned char *out, vec128 in) {
    PQCLEAN_MCELIECE6960119F_AVX_store8(out + 0, PQCLEAN_MCELIECE6960119F_AVX_vec128_extract(in, 0));
    PQCLEAN_MCELIECE6960119F_AVX_store8(out + 8, PQCLEAN_MCELIECE6960119F_AVX_vec128_extract(in, 1));
}
