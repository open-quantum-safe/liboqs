...
#if defined(OQS_ENABLE_KEM_classic_mceliece_8192128_vec)
int PQCLEAN_MCELIECE8192128_VEC_crypto_kem_keypair(unsigned char *pk, unsigned char *sk);
int PQCLEAN_MCELIECE8192128_VEC_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk);
int PQCLEAN_MCELIECE8192128_VEC_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk);
#endif
...
OQS_API OQS_STATUS OQS_KEM_classic_mceliece_8192128_encaps(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key) {
	...
	return (OQS_STATUS) PQCLEAN_MCELIECE8192128_VEC_crypto_kem_enc(ciphertext, shared_secret, public_key);
}
