...
static inline void vec_cmov(vec *out, const vec *in, uint16_t mask) {
    ...
    m0 = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(mask);
    m1 = ~m0;
...
static inline void get_coefs(gf *out, const vec *in) {
    ...
    mask[0][0] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0x5555);
    mask[0][1] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0xAAAA);
    mask[1][0] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0x3333);
    mask[1][1] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0xCCCC);
    mask[2][0] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0x0F0F);
    mask[2][1] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0xF0F0);
    mask[3][0] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0x00FF);
    mask[3][1] = PQCLEAN_MCELIECE8192128_VEC_vec_set1_16b(0xFF00);
...
void PQCLEAN_MCELIECE8192128_VEC_bm(vec out[][ GFBITS ], vec in[][ GFBITS ]) {
    int i;
    uint16_t N, L;
    uint16_t mask;
    uint64_t one = 1, t;
    ...
    for (N = 0; N < 256; N++) {
        PQCLEAN_MCELIECE8192128_VEC_vec_mul(prod[0], C[0], interval[0]);
        PQCLEAN_MCELIECE8192128_VEC_vec_mul(prod[1], C[1], interval[1]);
        update(interval, coefs[N]);
        d = vec_reduce(prod);

        t = PQCLEAN_MCELIECE8192128_VEC_gf_mul2(c0, coefs[N], b);
        d ^= t & 0xFFFFFFFF;

        mask = mask_nonzero(d) & mask_leq(L * 2, N);

        for (i = 0; i < GFBITS; i++) {
            dd[0][i] = dd[1][i] = PQCLEAN_MCELIECE8192128_VEC_vec_setbits((d >> i) & 1);
            bb[0][i] = bb[1][i] = PQCLEAN_MCELIECE8192128_VEC_vec_setbits((b >> i) & 1);
        }

        PQCLEAN_MCELIECE8192128_VEC_vec_mul(B_tmp[0], dd[0], B[0]);
        PQCLEAN_MCELIECE8192128_VEC_vec_mul(B_tmp[1], dd[1], B[1]);
        PQCLEAN_MCELIECE8192128_VEC_vec_mul(C_tmp[0], bb[0], C[0]);
        PQCLEAN_MCELIECE8192128_VEC_vec_mul(C_tmp[1], bb[1], C[1]);
    ...
    c0 = PQCLEAN_MCELIECE8192128_VEC_gf_inv(c0);

    for (i = 0; i < GFBITS; i++) {
        v[i] = PQCLEAN_MCELIECE8192128_VEC_vec_setbits((c0 >> i) & 1);
    }

    PQCLEAN_MCELIECE8192128_VEC_vec_mul(out[0], C[0], v);
    PQCLEAN_MCELIECE8192128_VEC_vec_mul(out[1], C[1], v);
}
