...
typedef struct {
	ALIGN(64)
	block_t A[MAX_MB_SLICES];
	ALIGN(64)
	block_t B[MAX_MB_SLICES];
	ALIGN(64)
	block_t C[MAX_MB_SLICES];
	ALIGN(64)
	block_t D[MAX_MB_SLICES];
	ALIGN(64)
	block_t E[MAX_MB_SLICES];
	ALIGN(64)
	block_t F[MAX_MB_SLICES];
	ALIGN(64)
	block_t G[MAX_MB_SLICES];
	ALIGN(64)
	block_t H[MAX_MB_SLICES];
	block_t Nl, Nh;
	uint64_t num; /* 1 or 2 */
...
_INLINE_ void sha_mb_avx(OUT sha_mb_ctx *ctx,
	...
	sha384_512_multi_block_avx2(ctx, input, min_length, secure_buf);
