...
OQS_KEM *OQS_KEM_kyber_1024_90s_new(void) {
	...
	kem->length_randomness = OQS_KEM_kyber_1024_90s_length_randomness;
	kem->derive_keypair = OQS_KEM_kyber_1024_90s_derive_keypair;
#endif
...
#if defined(OQS_HAZARDOUS_ENABLE_DERIVE_KEYPAIR)
extern int pqcrystals_kyber1024_90s_ref_derive_keypair(const uint8_t *randomness, uint8_t *pk, uint8_t *sk);
#endif
...
#if defined(OQS_HAZARDOUS_ENABLE_DERIVE_KEYPAIR)
extern int pqcrystals_kyber1024_90s_avx2_derive_keypair(const uint8_t *randomness, uint8_t *pk, uint8_t *sk);
#endif
...
OQS_API OQS_STATUS OQS_KEM_kyber_1024_90s_derive_keypair(const uint8_t *randomness, uint8_t *public_key, uint8_t *secret_key) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AES) && OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) pqcrystals_kyber1024_90s_avx2_derive_keypair(randomness, public_key, secret_key);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) pqcrystals_kyber1024_90s_ref_derive_keypair(randomness, public_key, secret_key);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) pqcrystals_kyber1024_90s_ref_derive_keypair(randomness, public_key, secret_key);
