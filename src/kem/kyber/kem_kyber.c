...
OQS_API OQS_STATUS OQS_KEM_kyber_768_cca_kem_keypair(uint8_t *public_key,
                                                     uint8_t *secret_key) {
	return (OQS_STATUS) PQCLEAN_KYBER768_crypto_kem_keypair(public_key, secret_key);
}
OQS_API OQS_STATUS OQS_KEM_kyber_768_cca_kem_encaps(uint8_t *ciphertext,
                                                    uint8_t *shared_secret,
                                                    const uint8_t *public_key) {
	return (OQS_STATUS) PQCLEAN_KYBER768_crypto_kem_enc(ciphertext, shared_secret, public_key);
}
OQS_API OQS_STATUS OQS_KEM_kyber_768_cca_kem_decaps(uint8_t *shared_secret,
                                                    const unsigned char *ciphertext,
                                                    const uint8_t *secret_key) {
	return (OQS_STATUS) PQCLEAN_KYBER768_crypto_kem_dec(shared_secret, ciphertext, secret_key);
}
