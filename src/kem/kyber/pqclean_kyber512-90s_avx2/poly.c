...
*              by PQCLEAN_KYBER51290S_AVX2_poly_reduce().
*
* Arguments:   - uint8_t *r: pointer to output byte array
*                            (of length KYBER_POLYCOMPRESSEDBYTES)
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_KYBER51290S_AVX2_poly_compress(uint8_t r[128], const poly *restrict a) {
    unsigned int i;
    __m256i f0, f1, f2, f3;
...
void PQCLEAN_KYBER51290S_AVX2_poly_decompress(poly *restrict r, const uint8_t a[128]) {
    unsigned int i;
    __m128i t;
    __m256i f;
    const __m256i q = _mm256_load_si256(&PQCLEAN_KYBER51290S_AVX2_qdata.vec[_16XQ / 16]);
...
*              The coefficients of the input polynomial are assumed to
*              lie in the invertal [0,q], i.e. the polynomial must be reduced
*              by PQCLEAN_KYBER51290S_AVX2_poly_reduce(). The coefficients are orderd as output by
*              PQCLEAN_KYBER51290S_AVX2_poly_ntt(); the serialized output coefficients are in bitreversed
*              order.
*
* Arguments:   - uint8_t *r: pointer to output byte array
...
void PQCLEAN_KYBER51290S_AVX2_poly_frombytes(poly *r, const uint8_t a[KYBER_POLYBYTES]) {
    PQCLEAN_KYBER51290S_AVX2_nttfrombytes_avx(r->vec, a, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}
...
**************************************************/
void PQCLEAN_KYBER51290S_AVX2_poly_frommsg(poly *restrict r, const uint8_t msg[KYBER_INDCPA_MSGBYTES]) {
...
void PQCLEAN_KYBER51290S_AVX2_poly_getnoise_eta1(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce) {
    ALIGNED_UINT8(KYBER_ETA1 * KYBER_N / 4 + 32) buf; // +32 bytes as required by PQCLEAN_KYBER51290S_AVX2_poly_cbd_eta1
    prf(buf.coeffs, KYBER_ETA1 * KYBER_N / 4, seed, nonce);
    PQCLEAN_KYBER51290S_AVX2_poly_cbd_eta1(r, buf.vec);
}

/*************************************************
* Name:        PQCLEAN_KYBER51290S_AVX2_poly_getnoise_eta2
*
* Description: Sample a polynomial deterministically from a seed and a nonce,
*              with output polynomial close to centered binomial distribution
...
void PQCLEAN_KYBER51290S_AVX2_poly_ntt(poly *r) {
    PQCLEAN_KYBER51290S_AVX2_ntt_avx(r->vec, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}
...
void PQCLEAN_KYBER51290S_AVX2_poly_invntt_tomont(poly *r) {
    PQCLEAN_KYBER51290S_AVX2_invntt_avx(r->vec, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}

void PQCLEAN_KYBER51290S_AVX2_poly_nttunpack(poly *r) {
    PQCLEAN_KYBER51290S_AVX2_nttunpack_avx(r->vec, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}
...
void PQCLEAN_KYBER51290S_AVX2_poly_basemul_montgomery(poly *r, const poly *a, const poly *b) {
    PQCLEAN_KYBER51290S_AVX2_basemul_avx(r->vec, a->vec, b->vec, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}
...
void PQCLEAN_KYBER51290S_AVX2_poly_tomont(poly *r) {
    PQCLEAN_KYBER51290S_AVX2_tomont_avx(r->vec, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}
...
void PQCLEAN_KYBER51290S_AVX2_poly_reduce(poly *r) {
    PQCLEAN_KYBER51290S_AVX2_reduce_avx(r->vec, PQCLEAN_KYBER51290S_AVX2_qdata.vec);
}
