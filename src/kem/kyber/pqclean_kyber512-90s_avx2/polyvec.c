...
**************************************************/
void PQCLEAN_KYBER51290S_AVX2_polyvec_decompress(polyvec *r, const uint8_t a[KYBER_POLYVECCOMPRESSEDBYTES + 12]) {
    size_t i;
...
/*************************************************
* Name:        PQCLEAN_KYBER51290S_AVX2_polyvec_basemul_acc_montgomery
*
...
**************************************************/
void PQCLEAN_KYBER51290S_AVX2_polyvec_basemul_acc_montgomery(poly *r, const polyvec *a, const polyvec *b) {
    size_t i;
    poly tmp;

    PQCLEAN_KYBER51290S_AVX2_poly_basemul_montgomery(r, &a->vec[0], &b->vec[0]);
    for (i = 1; i < KYBER_K; i++) {
        PQCLEAN_KYBER51290S_AVX2_poly_basemul_montgomery(&tmp, &a->vec[i], &b->vec[i]);
        PQCLEAN_KYBER51290S_AVX2_poly_add(r, r, &tmp);
    }
}
