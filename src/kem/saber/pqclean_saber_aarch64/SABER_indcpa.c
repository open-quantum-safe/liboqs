...
extern void PQCLEAN_SABER_AARCH64_asm_round(uint16_t des[SABER_N], uint32_t src[SABER_N]);
extern void PQCLEAN_SABER_AARCH64_asm_enc_add_msg(uint16_t cipher[SABER_N], uint32_t src[SABER_N], uint16_t msg[SABER_N], int const_h1);
extern void PQCLEAN_SABER_AARCH64_asm_dec_get_msg(uint16_t msg[SABER_N], uint32_t src[SABER_N], uint16_t cipher[SABER_N], int const_h2);
...
void indcpa_kem_keypair(uint8_t pk[SABER_INDCPA_PUBLICKEYBYTES], uint8_t sk[SABER_INDCPA_SECRETKEYBYTES]) {
    ...
    for (int i = 0; i < SABER_L; i++) {
        for (int j = 0; j < SABER_L; j++) {
            PQCLEAN_SABER_AARCH64_asm_13_to_32(&(A_NTT[j][i][0]), shake_A_buf + (i * SABER_L + j) * SABER_POLYBYTES);
        }
    }

    for (int i = 0; i < SABER_L; i++) {
        cbd(s[i], shake_s_buf + i * SABER_POLYCOINBYTES);
        PQCLEAN_SABER_AARCH64_asm_16_to_32(&(s_NTT[i][0]), &(s[i][0]));
    }
    ...
    for (int i = 0; i < SABER_L; i++) {
        PQCLEAN_SABER_AARCH64_asm_asymmetric_mul(&(A_NTT[i][0][0]), &(s_NTT[0][0]), &(s_NTT_asymmetric[0][0]), constants);
    }
    ...
    for (int i = 0; i < SABER_L; i++) {
        PQCLEAN_SABER_AARCH64_asm_round(b[i], A_NTT[i][0]);
    }
...
void indcpa_kem_enc(const uint8_t m[SABER_KEYBYTES], const uint8_t seed_sp[SABER_NOISE_SEEDBYTES], const uint8_t pk[SABER_INDCPA_PUBLICKEYBYTES], uint8_t ciphertext[SABER_BYTES_CCA_DEC]) {
    ...
    for (int i = 0; i < SABER_L; i++) {
        for (int j = 0; j < SABER_L; j++) {
            PQCLEAN_SABER_AARCH64_asm_13_to_32(&(A_NTT[i][j][0]), shake_A_buf + (i * SABER_L + j) * SABER_POLYBYTES);
        }
    }

    for (int i = 0; i < SABER_L; i++) {
        cbd(sp[i], shake_s_buf + i * SABER_POLYCOINBYTES);
        PQCLEAN_SABER_AARCH64_asm_16_to_32(&(s_NTT[i][0]), &(sp[i][0]));
    }
    ...
    for (int i = 0; i < SABER_L; i++) {
        PQCLEAN_SABER_AARCH64_asm_round(bp[i], A_NTT[i][0]);
    }
    ...
    for (int i = 0; i < SABER_L; i++) {
        PQCLEAN_SABER_AARCH64_asm_16_to_32(&(b_NTT[i][0]), &(b[i][0]));
    }
    ...
    PQCLEAN_SABER_AARCH64_asm_asymmetric_mul(&(b_NTT[0][0]), &(s_NTT[0][0]), &(s_NTT_asymmetric[0][0]), constants);

    iNTT(&(b_NTT[0][0]));

    PQCLEAN_SABER_AARCH64_asm_enc_add_msg(vp, b_NTT[0], mp, h1);
...
void indcpa_kem_dec(const uint8_t sk[SABER_INDCPA_SECRETKEYBYTES], const uint8_t ciphertext[SABER_BYTES_CCA_DEC], uint8_t m[SABER_KEYBYTES]) {
    ...
    for (int i = 0; i < SABER_L; i++) {
        PQCLEAN_SABER_AARCH64_asm_13_to_32(&(s_NTT[i][0]), sk + i * SABER_POLYBYTES);
    }

    for (int i = 0; i < SABER_L; i++) {
        PQCLEAN_SABER_AARCH64_asm_10_to_32(&(b_NTT[i][0]), ciphertext + i * (SABER_EP * SABER_N / 8));
    }
    ...
    PQCLEAN_SABER_AARCH64_asm_dec_get_msg(v, b_NTT[0], cm, h2);
