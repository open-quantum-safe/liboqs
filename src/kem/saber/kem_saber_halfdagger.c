...
#if defined(OQS_ENABLE_KEM_saber_halfdagger)

OQS_KEM *OQS_KEM_saber_halfdagger_new() {

	OQS_KEM *kem = malloc(sizeof(OQS_KEM));
	if (kem == NULL) {
		return NULL;
	}
	kem->method_name = OQS_KEM_alg_saber_halfdagger;
	...
	kem->length_public_key = OQS_KEM_saber_halfdagger_length_public_key;
	kem->length_secret_key = OQS_KEM_saber_halfdagger_length_secret_key;
	kem->length_ciphertext = OQS_KEM_saber_halfdagger_length_ciphertext;
	kem->length_shared_secret = OQS_KEM_saber_halfdagger_length_shared_secret;

	kem->keypair = OQS_KEM_saber_halfdagger_keypair;
	kem->encaps = OQS_KEM_saber_halfdagger_encaps;
	kem->decaps = OQS_KEM_saber_halfdagger_decaps;
...
#if defined(OQS_ENABLE_KEM_saber_halfdagger_avx2)
