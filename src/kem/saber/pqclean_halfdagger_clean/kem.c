...
int PQCLEAN_HALFDAGGER_CLEAN_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    size_t i;

    PQCLEAN_HALFDAGGER_CLEAN_indcpa_kem_keypair(pk, sk); // sk[0:SABER_INDCPA_SECRETKEYBYTES-1] <-- sk
    ...
    // This is output when check in PQCLEAN_HALFDAGGER_CLEAN_crypto_kem_dec() fails.
    return (0);
}

int PQCLEAN_HALFDAGGER_CLEAN_crypto_kem_enc(uint8_t *c, uint8_t *k, const uint8_t *pk) {
    ...
    PQCLEAN_HALFDAGGER_CLEAN_indcpa_kem_enc(c, buf, kr + 32, pk); // buf[0:31] contains message; kr[32:63] contains randomness r;
...
int PQCLEAN_HALFDAGGER_CLEAN_crypto_kem_dec(uint8_t *k, const uint8_t *c, const uint8_t *sk) {
    size_t i;
    uint8_t fail;
    ...
    PQCLEAN_HALFDAGGER_CLEAN_indcpa_kem_dec(buf, sk, c); // buf[0:31] <-- message
    ...
    PQCLEAN_HALFDAGGER_CLEAN_indcpa_kem_enc(cmp, buf, kr + 32, pk);

    fail = PQCLEAN_HALFDAGGER_CLEAN_verify(c, cmp, SABER_BYTES_CCA_DEC);

    sha3_256(kr + 32, c, SABER_BYTES_CCA_DEC); // overwrite coins in kr with h(c)

    PQCLEAN_HALFDAGGER_CLEAN_cmov(kr, sk + SABER_SECRETKEYBYTES - SABER_KEYBYTES, SABER_KEYBYTES, fail);
