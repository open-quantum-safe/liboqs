...
OQS_API const char *OQS_KEM_alg_identifier(size_t i) {
	...
	const char *a[OQS_KEM_algs_length] = {
	    ...
	    OQS_KEM_alg_frodo_640_aes, OQS_KEM_alg_frodo_640_cshake, OQS_KEM_alg_frodo_976_aes, OQS_KEM_alg_frodo_976_cshake,
...
OQS_API OQS_KEM *OQS_KEM_new(const char *method_name) {
	if (0 == strcasecmp(method_name, OQS_KEM_alg_default)) {
		...
		return NULL;
#endif
	} else if (0 == strcasecmp(method_name, OQS_KEM_alg_frodo_640_aes)) {
#ifdef OQS_ENABLE_KEM_frodo_640_aes
		return OQS_KEM_frodo_640_aes_new();
#else
		return NULL;
#endif
	} else if (0 == strcasecmp(method_name, OQS_KEM_alg_frodo_976_aes)) {
#ifdef OQS_ENABLE_KEM_frodo_976_aes
		return OQS_KEM_frodo_976_aes_new();
#else
		return NULL;
#endif
	} else if (0 == strcasecmp(method_name, OQS_KEM_alg_frodo_640_cshake)) {
#ifdef OQS_ENABLE_KEM_frodo_640_cshake
		return OQS_KEM_frodo_640_cshake_new();
#else
		return NULL;
#endif
	} else if (0 == strcasecmp(method_name, OQS_KEM_alg_frodo_976_cshake)) {
#ifdef OQS_ENABLE_KEM_frodo_976_cshake
		return OQS_KEM_frodo_976_cshake_new();
#else
		return NULL;
#endif
