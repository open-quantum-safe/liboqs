...
static int8_t F3_freeze(int16_t x) {
    return (int8_t) (PQCLEAN_NTRULPR761_CLEAN_int32_mod_uint14(x + 1, 3) - 1);
}
...
static Fq Fq_freeze(int32_t x) {
    return PQCLEAN_NTRULPR761_CLEAN_int32_mod_uint14(x + q12, q) - q12;
}
...
static void Short_fromlist(int8_t *out, const uint32_t *in) {
    ...
    PQCLEAN_NTRULPR761_CLEAN_uint32_sort(L, p);
    for (i = 0; i < p; ++i) {
        out[i] = (L[i] & 3) - 1;
    }
}
...
static void Generator(Fq *G, const unsigned char *k) {
    ...
    Expand(L, k);
    for (i = 0; i < p; ++i) {
        G[i] = PQCLEAN_NTRULPR761_CLEAN_uint32_mod_uint14(L[i], q) - q12;
    }
}
...
static void Rounded_encode(unsigned char *s, const Fq *r) {
    ...
    PQCLEAN_NTRULPR761_CLEAN_Encode(s, R, M, p);
}
...
static void Rounded_decode(Fq *r, const unsigned char *s) {
    ...
    PQCLEAN_NTRULPR761_CLEAN_Decode(R, s, M, p);
    for (i = 0; i < p; ++i) {
        r[i] = R[i] * 3 - q12;
    }
}
