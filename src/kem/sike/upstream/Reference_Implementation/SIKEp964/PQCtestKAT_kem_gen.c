...
static void print_hex(const char* title, const unsigned char* seq, size_t seqLen) {
  printf("%s", title);
  ...
  //      if(memcmp(sk,sk_rsp,CRYPTO_SECRETKEYBYTES)!=0){
	...
	if(memcmp(ss,ss_rsp,CRYPTO_BYTES)!=0){
         ...
         */
			...
			printf("count = %d\n", count);
			print_hex("seed = ", seed, 48);
			print_hex("pk = ", pk, CRYPTO_PUBLICKEYBYTES);
			print_hex("sk = ", sk, CRYPTO_SECRETKEYBYTES);
			print_hex("ct = ", ct, CRYPTO_CIPHERTEXTBYTES);
			print_hex("ss = ", ss, CRYPTO_BYTES);
      printf("\n");
