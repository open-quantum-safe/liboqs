...
// OQS note: size #defines moved to P610.c to avoid redefinitions across parameters

// SIKE's key generation
// It produces a private key sk and computes the public key pk.
// Outputs: secret key sk (CRYPTO_SECRETKEYBYTES = 524 bytes)
//          public key pk (CRYPTO_PUBLICKEYBYTES = 462 bytes)
int OQS_KEM_sike_p610_keypair(unsigned char *pk, unsigned char *sk);
...
int OQS_KEM_sike_p610_encaps(unsigned char *ct, unsigned char *ss, const unsigned char *pk);

// SIKE's decapsulation
// Input:   secret key sk         (CRYPTO_SECRETKEYBYTES = 524 bytes)
//          ciphertext message ct (CRYPTO_CIPHERTEXTBYTES = 486 bytes)
// Outputs: shared secret ss      (CRYPTO_BYTES = 24 bytes)
int OQS_KEM_sike_p610_decaps(unsigned char *ss, const unsigned char *ct, const unsigned char *sk);
...
void oqs_kem_sidh_p610_random_mod_order_A(unsigned char *random_digits);

// Generation of Bob's secret key
// Outputs random value in [0, 2^Floor(Log(2,3^192)) - 1] to be used as Bob's private key
void oqs_kem_sidh_p610_random_mod_order_B(unsigned char *random_digits);
...
int oqs_kem_sidh_p610_EphemeralKeyGeneration_A(const unsigned char *PrivateKeyA, unsigned char *PublicKeyA);
...
int oqs_kem_sidh_p610_EphemeralKeyGeneration_B(const unsigned char *PrivateKeyB, unsigned char *PublicKeyB);
...
int oqs_kem_sidh_p610_EphemeralSecretAgreement_A(const unsigned char *PrivateKeyA, const unsigned char *PublicKeyB, unsigned char *SharedSecretA);
...
int oqs_kem_sidh_p610_EphemeralSecretAgreement_B(const unsigned char *PrivateKeyB, const unsigned char *PublicKeyA, unsigned char *SharedSecretB);
