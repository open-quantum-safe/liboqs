...
static void to_mont(const felm_t a, felm_t mc) { // Conversion to Montgomery representation,
...
static void from_mont(const felm_t ma, felm_t c) { // Conversion from Montgomery representation to standard representation,
...
static void copy_words(const digit_t *a, digit_t *c, const unsigned int nwords) { // Copy wordsize digits, c = a, where lng(a) = nwords.
	unsigned int i;
...
static void fpmul_mont(const felm_t ma, const felm_t mb, felm_t mc) { // Multiprecision multiplication, c = a*b mod p.
	dfelm_t temp = {0};
...
static void fpsqr_mont(const felm_t ma, felm_t mc) { // Multiprecision squaring, c = a^2 mod p.
	dfelm_t temp = {0};
...
static void fpinv_mont(felm_t a) { // Field inversion using Montgomery arithmetic, a = a^(-1)*R mod p.
	felm_t tt;
...
static void fp2copy(const f2elm_t a, f2elm_t c) { // Copy a GF(p^2) element, c = a.
	fpcopy(a[0], c[0]);
	fpcopy(a[1], c[1]);
}

static void fp2zero(f2elm_t a) { // Zero a GF(p^2) element, a = 0.
	fpzero(a[0]);
	fpzero(a[1]);
}

static void fp2neg(f2elm_t a) { // GF(p^2) negation, a = -a in GF(p^2).
	fpneg(a[0]);
	fpneg(a[1]);
}
...
static void fp2div2(const f2elm_t a, f2elm_t c) { // GF(p^2) division by two, c = a/2  in GF(p^2).
	fpdiv2(a[0], c[0]);
	fpdiv2(a[1], c[1]);
}

static void fp2correction(f2elm_t a) { // Modular correction, a = a in GF(p^2).
	fpcorrection(a[0]);
	fpcorrection(a[1]);
}
...
static void fp2sqr_mont(const f2elm_t a, f2elm_t c) { // GF(p^2) squaring using Montgomery arithmetic, c = a^2 in GF(p^2).
...
static void fp2mul_mont(const f2elm_t a, const f2elm_t b, f2elm_t c) { // GF(p^2) multiplication using Montgomery arithmetic, c = a*b in GF(p^2).
...
static void fpinv_chain_mont(felm_t a) { // Chain to compute a^(p-3)/4 using Montgomery arithmetic.
	unsigned int i, j;
...
static void fp2inv_mont(f2elm_t a) { // GF(p^2) inversion using Montgomery arithmetic, a = (a0-i*a1)/(a0^2+a1^2).
	f2elm_t t1;
...
static void to_fp2mont(const f2elm_t a, f2elm_t mc) { // Conversion of a GF(p^2) element to Montgomery representation,
...
static void from_fp2mont(const f2elm_t ma, f2elm_t c) { // Conversion of a GF(p^2) element from Montgomery representation to standard representation,
...
static void mp_shiftr1(digit_t *x, const unsigned int nwords) { // Multiprecision right shift by one.
	unsigned int i;
...
static void mp_shiftl1(digit_t *x, const unsigned int nwords) { // Multiprecision left shift by one.
	int i;
...
static __inline unsigned int is_felm_zero(const felm_t x) { // Is x = 0? return 1 (TRUE) if condition is true, 0 (FALSE) otherwise.
...
static void mul3(unsigned char *a) { // Computes a = 3*a
...
static void fp2shl(const f2elm_t a, const int k, f2elm_t c) { // c = (2^k)*a
	fp2copy(a, c);
	for (int j = 0; j < k; j++) {
		fp2add(c, c, c);
	}
...
static void fp2_conj(const f2elm_t v, f2elm_t r) { // r = a - b*i where v = a + b*i
	fpcopy(v[0], r[0]);
	fpcopy(v[1], r[1]);
...
static void sqr_Fp2_cycl(f2elm_t a, const felm_t one) { // Cyclotomic squaring on elements of norm 1, using a^(p+1) = 1.
	felm_t t0;
...
static void cube_Fp2_cycl(f2elm_t a, const felm_t one) { // Cyclotomic cubing on elements of norm 1, using a^(p+1) = 1.
	felm_t t0;
...
static unsigned char is_sqr_fp2(const f2elm_t a, felm_t s) { // Test if a is a square in GF(p^2) and return 1 if true, 0 otherwise
...
static void sqrt_Fp2(const f2elm_t u, f2elm_t y) { // Computes square roots of elements in (Fp2)^2 using Hamburg's trick.
	felm_t t0, t1, t2, t3;
...
static __inline unsigned int is_felm_even(const felm_t x) { // Is x even? return 1 (TRUE) if condition is true, 0 (FALSE) otherwise.
	return (unsigned int) ((x[0] & 1) ^ 1);
}

static __inline unsigned int is_felm_lt(const felm_t x, const felm_t y) { // Is x < y? return 1 (TRUE) if condition is true, 0 (FALSE) otherwise.
	// SECURITY NOTE: This function does not run in constant-time.
	int i;
...
static __inline void fpinv_mont_bingcd_partial(const felm_t a, felm_t x1, unsigned int *k) { // Partial Montgomery inversion via the binary GCD algorithm.
	felm_t u, v, x2;
...
static void fpinv_mont_bingcd(felm_t a) { // Field inversion via the binary GCD using Montgomery arithmetic, a = a^-1*r' mod p.
...
static void fp2inv_mont_bingcd(f2elm_t a) { // GF(p^2) inversion using Montgomery arithmetic, a = (a0-i*a1)/(a0^2+a1^2)
...
static void mont_n_way_inv(const f2elm_t *vec, const int n, f2elm_t *out) { // n-way simultaneous inversion using Montgomery's trick.
	// SECURITY NOTE: This function does not run in constant time.
...
static void multiply(const digit_t *a, const digit_t *b, digit_t *c, const unsigned int nwords) { // Multiprecision comba multiply, c = a*b, where lng(a) = lng(b) = nwords.
...
static void Montgomery_neg(digit_t *a, digit_t *order) { // Modular negation, a = -a mod p.
...
static void Montgomery_multiply_mod_order(const digit_t *ma, const digit_t *mb, digit_t *mc, const digit_t *order, const digit_t *Montgomery_rprime) { // Montgomery multiplication modulo the group order, mc = ma*mb*r' mod order, where ma,mb,mc in [0, order-1].
...
static void to_Montgomery_mod_order(const digit_t *a, digit_t *mc, const digit_t *order, const digit_t *Montgomery_rprime, const digit_t *Montgomery_Rprime) { // Conversion of elements in Z_r to Montgomery representation, where the order r is up to NBITS_ORDER bits.
	Montgomery_multiply_mod_order(a, Montgomery_Rprime, mc, order, Montgomery_rprime);
}

static void from_Montgomery_mod_order(const digit_t *ma, digit_t *c, const digit_t *order, const digit_t *Montgomery_rprime) { // Conversion of elements in Z_r from Montgomery to standard representation, where the order is up to NBITS_ORDER bits.
...
static void Montgomery_inversion_mod_order_bingcd(const digit_t *a, digit_t *c, const digit_t *order, const digit_t *Montgomery_rprime, const digit_t *Montgomery_Rprime) { // Montgomery inversion modulo order, a = a^(-1)*R mod order.
...
static void inv_mod_orderA(const digit_t *a, digit_t *c) { // Inversion modulo an even integer of the form 2^m.
