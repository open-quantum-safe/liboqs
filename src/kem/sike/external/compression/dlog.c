...
static int ord2w_dlog(const felm_t *r, const int *logT, const felm_t *Texp) {
	...
	if (memcmp(x, y, NBITS_TO_NBYTES(NBITS_FIELD)) == 0) {
		return logT[1];
	}
	fpcopy(y, sum);
	fpneg(sum);
...
static int ord2w_dloghyb(const felm_t *h, const int *logT, const felm_t *Texp, const felm_t *G) {
	...
	fpcorrection(H[0][1]);
	if (is_felm_zero(H[0][1]) && k <= W_2_1) {
		return ord2w_dlog(H[k], logT, Texp) << w2;
	}
...
static void Traverse_w_div_e_torus(const felm_t *r, int j, int k, int z, const unsigned int *P, const felm_t *CT, int *D, int Dlen, int ellw, int w) {
...
// Assume the integer w divides the exponent e
	...
	if (z > 1) {
		int t = P[z];
		fp2copy(r, rp);
		for (int i = 0; i < (z - t)*w; i++) {
			sqr_Fp2_cycl_proj(rp);
		}
