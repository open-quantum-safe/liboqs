...
OQS_API OQS_STATUS OQS_SIG_STFL_alg_xmss_shake128_h10_keypair(XMSS_UNUSED_ATT uint8_t *public_key, XMSS_UNUSED_ATT uint8_t *secret_key) {

	if (public_key == NULL || secret_key == NULL) {
		return OQS_ERROR;
	}

	const uint32_t xmss_shake128_h10_oid = 0x07;
	if (xmss_keypair(public_key, secret_key, xmss_shake128_h10_oid)) {
		return OQS_ERROR;
	}
...
OQS_API OQS_STATUS OQS_SIG_STFL_alg_xmss_shake128_h10_sign(uint8_t *signature, size_t *signature_len, XMSS_UNUSED_ATT const uint8_t *message, XMSS_UNUSED_ATT size_t message_len, XMSS_UNUSED_ATT uint8_t *secret_key) {
