...
static int kex_test_correctness(OQS_RAND *rand, enum OQS_KEX_alg_name alg_name,
  ...
  if (alice_key_len != bob_key_len) {
    fprintf(stderr, "ERROR: Alice's session key and Bob's session key are "
                    "different lengths (%zu vs %zu)\n",
...
static int kex_bench_wrapper(OQS_RAND *rand, enum OQS_KEX_alg_name alg_name,
                             const uint8_t *seed, const size_t seed_len,
                             const char *named_parameters, const int seconds) {
