...
static int kex_test_correctness(OQS_RAND *rand,
                                OQS_KEX * (*new_method)(
                                    OQS_RAND *, const uint8_t *, const size_t,
                                    const char *, enum KEX_ALGO_NAMES),
                                const uint8_t *seed, const size_t seed_len,
                                const char *named_parameters,
                                const int print, unsigned long occurrences[256],
                                enum KEX_ALGO_NAMES kex_algo) {
	...
	/* setup KEX */
	kex = new_method(rand, seed, seed_len,
	                 named_parameters, kex_algo);
	if (kex == NULL) {
		goto err;
	}
	...
	rc = OQS_KEX_alice_0(kex, &alice_priv, &alice_msg,
	                     &alice_msg_len);
	if (rc != 1) {
		goto err;
	}
	...
	if (print) {
		PRINT_HEX_STRING("Alice message", alice_msg,
		                 alice_msg_len)
	}

	/* Bob's response */
	rc = OQS_KEX_bob(kex, alice_msg, alice_msg_len,
	                 &bob_msg, &bob_msg_len,
	                 &bob_key, &bob_key_len);
	if (rc != 1) {
		goto err;
	...
	if (print) {
		PRINT_HEX_STRING("Bob message", bob_msg,
		                 bob_msg_len)
		PRINT_HEX_STRING("Bob session key", bob_key,
		                 bob_key_len)
	}

	/* Alice processes Bob's response */
	rc = OQS_KEX_alice_1(kex, alice_priv, bob_msg,
	...
	if (print) {
		PRINT_HEX_STRING("Alice session key", alice_key,
		                 alice_key_len)
	}
	...
	for (size_t i = 0; i < alice_key_len; i++) {
		OQS_RAND_test_record_occurrence(alice_key[i],
		                                occurrences);
	}
...
static int kex_test_correctness_wrapper(
    OQS_RAND *rand,
    OQS_KEX * (*new_method)(OQS_RAND *,
	...
	ret = kex_test_correctness(rand, new_method, seed,
	                           seed_len, named_parameters,
...
int main() {
	...
	ret = kex_test_correctness_wrapper(rand,
	                                   &OQS_KEX_new, NULL, 0, NULL,
