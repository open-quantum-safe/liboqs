...
OQS_KEX *OQS_KEX_new(OQS_RAND *rand, const uint8_t *seed,
                     const size_t seed_len, const char *named_parameters,
                     enum KEX_ALGO_NAMES alg_name) {
...
int OQS_KEX_bob(OQS_KEX *k, const uint8_t *alice_msg,
	...
	if (k == NULL) {
		...
		return k->bob(k, alice_msg, alice_msg_len, bob_msg,
		              bob_msg_len, key, key_len);
	}
}

int OQS_KEX_alice_1(OQS_KEX *k, const void *alice_priv,
                    const uint8_t *bob_msg,
                    const size_t bob_msg_len, uint8_t **key, size_t *key_len) {
	if (k == NULL) {
		return 0;
	} else {
		return k->alice_1(k, alice_priv, bob_msg, bob_msg_len, key,
		                  key_len);
	}
}
