...
static OQS_RAND_urandom_chacha20_ctx
*OQS_RAND_urandom_chacha20_ctx_new();
static void OQS_RAND_urandom_chacha20_fill_cache(
    OQS_RAND *r);
static void OQS_RAND_urandom_chacha20_ctx_free(
    void *rand_ctx);
...
*OQS_RAND_urandom_chacha20_ctx_new() {
	...
	rand_ctx = (OQS_RAND_urandom_chacha20_ctx *)
	           malloc(sizeof(
	                      OQS_RAND_urandom_chacha20_ctx));
	if (rand_ctx == NULL) {
		goto err;
	...
	ECRYPT_keysetup(rand_ctx->chacha20_input,
	                rand_ctx->key);
	goto okay;
err:
	if (rand_ctx) {
...
uint8_t OQS_RAND_urandom_chacha20_8(OQS_RAND *r) {
	...
	uint8_t out =
	    rand_ctx->cache[rand_ctx->cache_next_byte];
...
uint32_t OQS_RAND_urandom_chacha20_32(
	...
	uint32_t out;
	memcpy(&out,
	       &rand_ctx->cache[rand_ctx->cache_next_byte], 4);
...
uint64_t OQS_RAND_urandom_chacha20_64(
	...
	uint64_t out;
	memcpy(&out,
	       &rand_ctx->cache[rand_ctx->cache_next_byte], 8);
...
void OQS_RAND_urandom_chacha20_n(OQS_RAND *r,
	...
	ECRYPT_ivsetup(rand_ctx->chacha20_input,
	               (u8 *) rand_ctx->nonce);
	ECRYPT_keystream_bytes(rand_ctx->chacha20_input,
	                       out, n);
}
