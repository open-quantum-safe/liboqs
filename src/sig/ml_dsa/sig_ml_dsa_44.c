...
extern int pqcrystals_ml_dsa_44_ref_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed);
...
extern int pqcrystals_ml_dsa_44_avx2_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *seed);
...
OQS_API OQS_STATUS OQS_SIG_ml_dsa_44_keypair_derand(uint8_t *public_key, uint8_t *secret_key, const uint8_t *seed) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) pqcrystals_ml_dsa_44_avx2_keypair_derand(public_key, secret_key, seed);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) pqcrystals_ml_dsa_44_ref_keypair_derand(public_key, secret_key, seed);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) pqcrystals_ml_dsa_44_ref_keypair_derand(public_key, secret_key, seed);
...
OQS_API OQS_STATUS OQS_SIG_ml_dsa_44_verify(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *public_key) {
	...
	if (signature_len != OQS_SIG_ml_dsa_44_length_signature) {
		return OQS_ERROR;
	}
