...
{
    assert(coord0 != INT32_MIN && coord1 != INT32_MIN && coord2 != INT32_MIN && coord3 != INT32_MIN);
    ibz_set(&(vec->v[0]), coord0, 32);
    ibz_set(&(vec->v[1]), coord1, 32);
    ibz_set(&(vec->v[2]), coord2, 32);
    ibz_set(&(vec->v[3]), coord3, 32);
}

void
ibz_vec_4_copy(ibz_vec_4_t *copy, const ibz_vec_4_t *copied)
{
    for (int i = 0; i < 4; i++) {
        ibz_copy(&(copy->v[i]), &(copied->v[i]));
    }
}
...
void
ibz_mat_4x4_copy(ibz_mat_4x4_t *copy, const ibz_mat_4x4_t *copied)
{
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ibz_copy(&(copy->m[i][j]), &(copied->m[i][j]));
        }
    }
}
...
ibz_mat_4x4_identity(ibz_mat_4x4_t *id)
{
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ibz_set(&(id->m[i][j]), 0, 0);
        }
        ibz_set(&(id->m[i][i]), 1, 2);
    }
}
...
void
ibz_mat_4x4_inv_make_coeff_pmp(ibz_t *coeff,
                               const ibz_t *a1,
                               const ibz_t *a2,
                               const ibz_t *b1,
                               const ibz_t *b2,
...
void
ibz_mat_4x4_inv_make_coeff_mpm(ibz_t *coeff,
                               const ibz_t *a1,
                               const ibz_t *a2,
                               const ibz_t *b1,
                               const ibz_t *b2,
...
ibz_mat_4x4_inv_with_det_as_denom(ibz_mat_4x4_t *inv, ibz_t *det, const ibz_mat_4x4_t *mat)
{
    ...
    for (int j = 0; j < 4; j++) {
        for (int k = 0; k < 2; k++) {
            if ((k + j + 1) % 2 == 1) {
                ibz_mat_4x4_inv_make_coeff_pmp(&(work.m[j][k]),
                                               &(mat->m[1 - k][(j == 0)]),
                                               &(c[6 - j - (j == 0)]),
                                               &(mat->m[1 - k][2 - (j > 1)]),
                                               &(c[4 - j - (j == 1)]),
                ...
                ibz_mat_4x4_inv_make_coeff_mpm(&(work.m[j][k]),
                                               &(mat->m[1 - k][(j == 0)]),
                                               &(c[6 - j - (j == 0)]),
                                               &(mat->m[1 - k][2 - (j > 1)]),
                                               &(c[4 - j - (j == 1)]),
