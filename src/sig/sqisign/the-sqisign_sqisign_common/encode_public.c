...
signature_to_bytes(byte_t *enc, const signature_t *sig)
{
    ...
    encode_digits(enc, sig->mat_Bchall_can_to_B_chall[0][0], RESPONSE_BYTES);
    enc += RESPONSE_BYTES;
    encode_digits(enc, sig->mat_Bchall_can_to_B_chall[0][1], RESPONSE_BYTES);
    enc += RESPONSE_BYTES;
    encode_digits(enc, sig->mat_Bchall_can_to_B_chall[1][0], RESPONSE_BYTES);
    enc += RESPONSE_BYTES;
    encode_digits(enc, sig->mat_Bchall_can_to_B_chall[1][1], RESPONSE_BYTES);
    enc += RESPONSE_BYTES;

    encode_digits(enc, sig->chall_coeff, CHALLENGE_BYTES);
    enc += CHALLENGE_BYTES;
...
signature_from_bytes(signature_t *sig, const byte_t *enc)
{
    ...
    decode_digits(sig->mat_Bchall_can_to_B_chall[0][0], enc, RESPONSE_BYTES, NWORDS_ORDER);
    enc += RESPONSE_BYTES;
    decode_digits(sig->mat_Bchall_can_to_B_chall[0][1], enc, RESPONSE_BYTES, NWORDS_ORDER);
    enc += RESPONSE_BYTES;
    decode_digits(sig->mat_Bchall_can_to_B_chall[1][0], enc, RESPONSE_BYTES, NWORDS_ORDER);
    enc += RESPONSE_BYTES;
    decode_digits(sig->mat_Bchall_can_to_B_chall[1][1], enc, RESPONSE_BYTES, NWORDS_ORDER);
    enc += RESPONSE_BYTES;

    decode_digits(sig->chall_coeff, enc, CHALLENGE_BYTES, NWORDS_ORDER);
    enc += CHALLENGE_BYTES;
