 ...
 */
void PQCLEAN_SPHINCSHARAKA256FROBUST_AESNI_prf_addrx4(unsigned char *out0,
        unsigned char *out1,
        unsigned char *out2,
        unsigned char *out3,
    ...
    unsigned char bufx4[4 * PQCLEAN_SPHINCSHARAKA256FROBUST_AESNI_ADDR_BYTES];
    ...
    for (i = 0; i < 4; i++) {
        PQCLEAN_SPHINCSHARAKA256FROBUST_AESNI_addr_to_bytes(bufx4 + i * PQCLEAN_SPHINCSHARAKA256FROBUST_AESNI_ADDR_BYTES, addrx4 + i * 8);
    }

    PQCLEAN_SPHINCSHARAKA256FROBUST_AESNI_haraka256_skx4(outbuf, bufx4, state_seeded);
