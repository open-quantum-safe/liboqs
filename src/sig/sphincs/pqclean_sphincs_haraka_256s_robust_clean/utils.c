...
void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_compute_root(
    ...
    uint32_t i;
    unsigned char buffer[2 * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N];

    /* If leaf_idx is odd (last bit = 1), current path element is a right child
       and auth_path has to go left. Otherwise it is the other way around. */
    if (leaf_idx & 1) {
        memcpy(buffer + PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, leaf, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
        memcpy(buffer, auth_path, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
    } else {
        memcpy(buffer, leaf, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
    ...
    for (i = 0; i < tree_height - 1; i++) {
        ...
        if (leaf_idx & 1) {
            PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_thash_2(
                buffer + PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, buffer, pub_seed, addr, hash_state_seeded);
            memcpy(buffer, auth_path, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
        } else {
            PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_thash_2(
 ...
 * tree type (i.e. PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_ADDR_TYPE_HASHTREE or PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_ADDR_TYPE_FORSTREE).
...
static void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_treehash(
    ...
    for (idx = 0; idx < (uint32_t)(1 << tree_height); idx++) {
        ...
        gen_leaf(stack + offset * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N,
        ...
        if ((leaf_idx ^ 0x1) == idx) {
            memcpy(auth_path, stack + (offset - 1)*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
        }
        ...
        while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {
            ...
            PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_thash_2(
                stack + (offset - 2)*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, stack + (offset - 2)*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N,
            ...
            if (((leaf_idx >> heights[offset - 1]) ^ 0x1) == tree_idx) {
                memcpy(auth_path + heights[offset - 1]*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N,
                       stack + (offset - 1)*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
            }
        }
    }
    memcpy(root, stack, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N);
}
...
void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_treehash_FORS_HEIGHT(
    ...
    unsigned char stack[(PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT + 1)*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N];
    unsigned int heights[PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT + 1];

    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_treehash(
        root, auth_path, stack, heights, sk_seed, pub_seed,
        leaf_idx, idx_offset, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT, gen_leaf, tree_addr, hash_state_seeded);
}
...
void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_treehash_TREE_HEIGHT(
    ...
    unsigned char stack[(PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_TREE_HEIGHT + 1)*PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N];
    unsigned int heights[PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_TREE_HEIGHT + 1];

    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_treehash(
        root, auth_path, stack, heights, sk_seed, pub_seed,
        leaf_idx, idx_offset, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_TREE_HEIGHT, gen_leaf, tree_addr, hash_state_seeded);
}
