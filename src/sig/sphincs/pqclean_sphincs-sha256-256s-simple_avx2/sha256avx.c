...
void PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_sha256_update8x(sha256ctxx8 *ctx,
    ...
    while (i < len) {
        ...
        i += bytes_to_copy;
        if (ctx->datalen == 64) {
            PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_sha256_transform8x(ctx, ctx->msgblocks);
            ctx->msglen += 512;
            ctx->datalen = 0;
        }
    }
...
void PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_sha256_final8x(sha256ctxx8 *ctx,
    ...
    PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_transpose(ctx->s);
...
void PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_sha256_transform8x(sha256ctxx8 *ctx, const unsigned char *data) {
    ...
    PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_transpose(w);
    PQCLEAN_SPHINCSSHA256256SSIMPLE_AVX2_transpose(w + 8);
