...
#define treehashx4_variant(name, tree_height)                                                      \
    void PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_treehashx4_##name(                                                                    \
        ...
        unsigned char stackx4[4 * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N];                                    \
        ...
        for (idx = 0; idx < (uint32_t)(1 << (tree_height)); idx++) {                               \
            ...
            gen_leafx4(stackx4 + 0 * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N + offset * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N,                 \
                       ...
                       stackx4 + 3 * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N + offset * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N, sk_seed,        \
            ...
            for (j = 0; j < 4; j++) {                                                              \
                if ((leaf_idx[j] ^ 0x1) == idx) {                                                  \
                    memcpy(auth_pathx4 + j * (tree_height)*PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N,                                  \
                           stackx4 + j * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N + (offset - 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N,       \
            ...
            while (offset >= 2 && heights[offset - 1] == heights[offset - 2]) {                    \
                ...
                for (j = 0; j < 4; j++) {                                                          \
                    PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_set_tree_height(tree_addrx4 + j * 8, heights[offset - 1] + 1);             \
                    PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_set_tree_index(tree_addrx4 + j * 8,                                        \
                            tree_idx + (idx_offset[j] >> (heights[offset - 1] + 1)));   \
                }                                                                                  \
                /* Hash the top-most nodes from the stack together. */                             \
                PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_thashx4_2(stackx4 + 0 * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N + (offset - 2) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N,    \
                ...
                for (j = 0; j < 4; j++) {                                                          \
                    if (((leaf_idx[j] >> heights[offset - 1]) ^ 0x1) == tree_idx) {                \
                        memcpy(auth_pathx4 + j * (tree_height)*PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N +                             \
                               heights[offset - 1] * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N,                                    \
                               stackx4 + j * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N + (offset - 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N,   \
                               PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N);                                                             \
                    }                                                                              \
                }                                                                                  \
        ...
        \
        for (j = 0; j < 4; j++) {                                                                  \
            memcpy(rootx4 + j * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N, stackx4 + j * ((tree_height) + 1) * PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N, PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_N);          \
        }                                                                                          \
    }
