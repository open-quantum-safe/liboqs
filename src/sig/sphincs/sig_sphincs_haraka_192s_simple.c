...
int PQCLEAN_SPHINCSHARAKA192SSIMPLE_AESNI_crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk);
int PQCLEAN_SPHINCSHARAKA192SSIMPLE_AESNI_crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m, size_t mlen, const uint8_t *pk);
#endif
...
OQS_API OQS_STATUS OQS_SIG_sphincs_haraka_192s_simple_keypair(uint8_t *public_key, uint8_t *secret_key) {
	...
	if (available_cpu_extensions.AES_ENABLED) {
		return (OQS_STATUS) PQCLEAN_SPHINCSHARAKA192SSIMPLE_AESNI_crypto_sign_keypair(public_key, secret_key);
	} else {
