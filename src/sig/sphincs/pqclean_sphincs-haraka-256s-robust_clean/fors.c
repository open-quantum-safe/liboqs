 ...
 * Assumes m contains at least PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES bits.
 * Assumes indices has space for PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES integers.
 */
static void message_to_indices(uint32_t *indices, const unsigned char *m) {
    unsigned int i, j;
    unsigned int offset = 0;

    for (i = 0; i < PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES; i++) {
        indices[i] = 0;
        for (j = 0; j < PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT; j++) {
...
void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_fors_sign(
    ...
    uint32_t indices[PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES];
    unsigned char roots[PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N];
    ...
    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_set_type(
        fors_tree_addr, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_ADDR_TYPE_FORSTREE);
    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_set_type(
        fors_pk_addr, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_ADDR_TYPE_FORSPK);

    message_to_indices(indices, m);

    for (i = 0; i < PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES; i++) {
        idx_offset = i * (1 << PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT);
        ...
        PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_treehash_FORS_HEIGHT(
            roots + i * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, sig, sk_seed, pub_seed,
            indices[i], idx_offset, fors_gen_leaf, fors_tree_addr,
            hash_state_seeded);
        sig += PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT;
    }
...
void PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_fors_pk_from_sig(
    ...
    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_copy_keypair_addr(fors_tree_addr, fors_addr);
    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_copy_keypair_addr(fors_pk_addr, fors_addr);

    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_set_type(fors_tree_addr, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_ADDR_TYPE_FORSTREE);
    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_set_type(fors_pk_addr, PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_ADDR_TYPE_FORSPK);
    ...
    for (i = 0; i < PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_TREES; i++) {
        ...
        PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_set_tree_height(fors_tree_addr, 0);
        PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_set_tree_index(fors_tree_addr, indices[i] + idx_offset);
        ...
        PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_compute_root(roots + i * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N, leaf, indices[i], idx_offset, sig,
                PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT, pub_seed, fors_tree_addr, hash_state_seeded);
        sig += PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_N * PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_FORS_HEIGHT;
    }

    /* Hash horizontally across all tree roots to derive the public key. */
    PQCLEAN_SPHINCSHARAKA256SROBUST_CLEAN_thash_FORS_TREES(pk, roots, pub_seed, fors_pk_addr, hash_state_seeded);
}
