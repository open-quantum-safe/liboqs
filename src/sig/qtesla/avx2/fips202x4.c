...
#include <stdint.h>
#include <assert.h>
// #include "fips202.h" provided by <oqs/sha3.h>
...
static void cshake128_simple_absorb4x(__m256i *s, uint16_t cstm0, uint16_t cstm1, uint16_t cstm2, uint16_t cstm3, const unsigned char *in, unsigned long long inlen) {
	...
	/* Absorb input */
	keccak_absorb4x(s, OQS_SHA3_SHAKE128_RATE, in, in, in, in, inlen, 0x04);
}
...
static void cshake128_simple_squeezeblocks4x(unsigned char *output0, unsigned char *output1, unsigned char *output2, unsigned char *output3, unsigned long long outlen, __m256i *s) {
	keccak_squeezeblocks4x(output0, output1, output2, output3, outlen / OQS_SHA3_SHAKE128_RATE, s, OQS_SHA3_SHAKE128_RATE);
}
#endif
...
static void cshake128_simple4x(unsigned char *output0, unsigned char *output1, unsigned char *output2, unsigned char *output3, unsigned long long outlen,
	...
	__m256i s[25];
	unsigned char t0[OQS_SHA3_SHAKE128_RATE];
	unsigned char t1[OQS_SHA3_SHAKE128_RATE];
	unsigned char t2[OQS_SHA3_SHAKE128_RATE];
	unsigned char t3[OQS_SHA3_SHAKE128_RATE];
	unsigned int i;
	...
	/* Squeeze output */
	keccak_squeezeblocks4x(output0, output1, output2, output3, outlen / OQS_SHA3_SHAKE128_RATE, s, OQS_SHA3_SHAKE128_RATE);
	output0 += (outlen / OQS_SHA3_SHAKE128_RATE) * OQS_SHA3_SHAKE128_RATE;
	output1 += (outlen / OQS_SHA3_SHAKE128_RATE) * OQS_SHA3_SHAKE128_RATE;
	output2 += (outlen / OQS_SHA3_SHAKE128_RATE) * OQS_SHA3_SHAKE128_RATE;
	output3 += (outlen / OQS_SHA3_SHAKE128_RATE) * OQS_SHA3_SHAKE128_RATE;

	if (outlen % OQS_SHA3_SHAKE128_RATE) {
		keccak_squeezeblocks4x(t0, t1, t2, t3, 1, s, OQS_SHA3_SHAKE128_RATE);
		for (i = 0; i < outlen % OQS_SHA3_SHAKE128_RATE; i++) {
			output0[i] = t0[i];
			output1[i] = t1[i];
			output2[i] = t2[i];
			output3[i] = t3[i];
...
static void cshake256_simple_absorb4x(__m256i *s, uint16_t cstm0, uint16_t cstm1, uint16_t cstm2, uint16_t cstm3, const unsigned char *in, unsigned long long inlen) {
	...
	/* Absorb input */
	keccak_absorb4x(s, OQS_SHA3_SHAKE256_RATE, in, in, in, in, inlen, 0x04);
}
...
static void cshake256_simple_squeezeblocks4x(unsigned char *output0, unsigned char *output1, unsigned char *output2, unsigned char *output3, unsigned long long outlen, __m256i *s) {
	keccak_squeezeblocks4x(output0, output1, output2, output3, outlen / OQS_SHA3_SHAKE256_RATE, s, OQS_SHA3_SHAKE256_RATE);
}
...
static void cshake256_simple4x(unsigned char *output0, unsigned char *output1, unsigned char *output2, unsigned char *output3, unsigned long long outlen,
                               uint16_t cstm0, uint16_t cstm1, uint16_t cstm2, uint16_t cstm3, const unsigned char *in, unsigned long long inlen) {
	__m256i s[25];
	unsigned char t0[OQS_SHA3_SHAKE256_RATE];
	unsigned char t1[OQS_SHA3_SHAKE256_RATE];
	unsigned char t2[OQS_SHA3_SHAKE256_RATE];
	unsigned char t3[OQS_SHA3_SHAKE256_RATE];
	unsigned int i;
	...
	/* Squeeze output */
	keccak_squeezeblocks4x(output0, output1, output2, output3, outlen / OQS_SHA3_SHAKE256_RATE, s, OQS_SHA3_SHAKE256_RATE);
	output0 += (outlen / OQS_SHA3_SHAKE256_RATE) * OQS_SHA3_SHAKE256_RATE;
	output1 += (outlen / OQS_SHA3_SHAKE256_RATE) * OQS_SHA3_SHAKE256_RATE;
	output2 += (outlen / OQS_SHA3_SHAKE256_RATE) * OQS_SHA3_SHAKE256_RATE;
	output3 += (outlen / OQS_SHA3_SHAKE256_RATE) * OQS_SHA3_SHAKE256_RATE;

	if (outlen % OQS_SHA3_SHAKE256_RATE) {
		keccak_squeezeblocks4x(t0, t1, t2, t3, 1, s, OQS_SHA3_SHAKE256_RATE);
		for (i = 0; i < outlen % OQS_SHA3_SHAKE256_RATE; i++) {
			output0[i] = t0[i];
			output1[i] = t1[i];
			output2[i] = t2[i];
			output3[i] = t3[i];
