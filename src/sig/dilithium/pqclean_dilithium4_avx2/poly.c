...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_reduce
*
...
void PQCLEAN_DILITHIUM4_AVX2_poly_reduce(poly *a) {
    PQCLEAN_DILITHIUM4_AVX2_reduce_avx(a->coeffs);
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_csubq
*
...
void PQCLEAN_DILITHIUM4_AVX2_poly_csubq(poly *a) {
    PQCLEAN_DILITHIUM4_AVX2_csubq_avx(a->coeffs);
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_freeze
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_add
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_sub
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_shiftl
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_ntt
*
...
void PQCLEAN_DILITHIUM4_AVX2_poly_ntt(poly *a) {
    ...
    for (size_t i = 0; i < N / 32; ++i) {
        PQCLEAN_DILITHIUM4_AVX2_ntt_levels0t2_avx(tmp.as_arr + 4 * i, a->coeffs + 4 * i, PQCLEAN_DILITHIUM4_AVX2_zetas.as_arr + 1);
    }
    for (size_t i = 0; i < N / 32; ++i) {
        PQCLEAN_DILITHIUM4_AVX2_ntt_levels3t8_avx(a->coeffs + 32 * i, tmp.as_arr + 32 * i, PQCLEAN_DILITHIUM4_AVX2_zetas.as_arr + 8 + 31 * i);
    }
}
...
void PQCLEAN_DILITHIUM4_AVX2_poly_invntt_montgomery(poly *a) {
    ...
    for (size_t i = 0; i < N / 32; i++) {
        PQCLEAN_DILITHIUM4_AVX2_invntt_levels0t4_avx(tmp.as_arr + 32 * i, a->coeffs + 32 * i, PQCLEAN_DILITHIUM4_AVX2_zetas_inv.as_arr + 31 * i);
    }
    for (size_t i = 0; i < N / 32; i++) {
        PQCLEAN_DILITHIUM4_AVX2_invntt_levels5t7_avx(a->coeffs + 4 * i, tmp.as_arr + 4 * i, PQCLEAN_DILITHIUM4_AVX2_zetas_inv.as_arr + 248);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_pointwise_invmontgomery
*
...
void PQCLEAN_DILITHIUM4_AVX2_poly_pointwise_invmontgomery(poly *c, const poly *a, const poly *b) {
    PQCLEAN_DILITHIUM4_AVX2_pointwise_avx(c->coeffs, a->coeffs, b->coeffs);
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_power2round
*
...
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_poly_power2round(poly *restrict a1,
        poly *restrict a0,
        const poly *restrict a) {
    for (size_t i = 0; i < N; ++i) {
        a1->coeffs[i] = PQCLEAN_DILITHIUM4_AVX2_power2round(a->coeffs[i], &a0->coeffs[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_decompose
*
* Description: For all coefficients c of the input polynomial,
...
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_poly_decompose(
    poly *restrict a1,
    poly *restrict a0,
    const poly *restrict a) {
    for (size_t i = 0; i < N; ++i) {
        a1->coeffs[i] = PQCLEAN_DILITHIUM4_AVX2_decompose(a->coeffs[i], &a0->coeffs[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_make_hint
*
...
**************************************************/
uint32_t PQCLEAN_DILITHIUM4_AVX2_poly_make_hint(
    poly *restrict h,
    const poly *restrict a0,
    const poly *restrict a1) {
    uint32_t s = 0;
    for (size_t i = 0; i < N; ++i) {
        h->coeffs[i] = PQCLEAN_DILITHIUM4_AVX2_make_hint(a0->coeffs[i], a1->coeffs[i]);
        s += h->coeffs[i];
    }
    return s;
}
...
/*************************************************
 * Name:       PQCLEAN_DILITHIUM4_AVX2_poly_use_hint
 *
...
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_poly_use_hint(
    poly *restrict a,
    const poly *restrict b,
    const poly *restrict h) {
    for (size_t i = 0; i < N; ++i) {
        a->coeffs[i] = PQCLEAN_DILITHIUM4_AVX2_use_hint(b->coeffs[i], h->coeffs[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_chknorm
*
...
void PQCLEAN_DILITHIUM4_AVX2_poly_uniform(poly *a,
    ...
    ctr = PQCLEAN_DILITHIUM4_AVX2_rej_uniform(a->coeffs, N, buf, buflen);
...
void PQCLEAN_DILITHIUM4_AVX2_poly_uniform_4x(poly *a0,
    ...
    PQCLEAN_DILITHIUM4_AVX2_shake128_absorb4x(state, inbuf[0], inbuf[1], inbuf[2], inbuf[3],
            SEEDBYTES + 2);
    PQCLEAN_DILITHIUM4_AVX2_shake128_squeezeblocks4x(outbuf[0], outbuf[1], outbuf[2], outbuf[3], 5,
            state);

    ctr0 = PQCLEAN_DILITHIUM4_AVX2_rej_uniform(a0->coeffs, N, outbuf[0], 5 * SHAKE128_RATE);
    ctr1 = PQCLEAN_DILITHIUM4_AVX2_rej_uniform(a1->coeffs, N, outbuf[1], 5 * SHAKE128_RATE);
    ...
    while (ctr0 < N || ctr1 < N || ctr2 < N || ctr3 < N) {
        PQCLEAN_DILITHIUM4_AVX2_shake128_squeezeblocks4x(outbuf[0], outbuf[1], outbuf[2], outbuf[3], 1,
                state);
...
void PQCLEAN_DILITHIUM4_AVX2_poly_uniform_eta(
    poly *a,
    const uint8_t seed[SEEDBYTES],
    uint16_t nonce) {
    size_t ctr;
    ...
    ctr = PQCLEAN_DILITHIUM4_AVX2_rej_eta(a->coeffs, N, buf, POLY_UNIFORM_ETA_BUFLEN);
...
void PQCLEAN_DILITHIUM4_AVX2_poly_uniform_eta_4x(
    ...
    PQCLEAN_DILITHIUM4_AVX2_shake128_absorb4x(state, inbuf[0], inbuf[1], inbuf[2], inbuf[3],
            SEEDBYTES + 2);
    PQCLEAN_DILITHIUM4_AVX2_shake128_squeezeblocks4x(outbuf[0], outbuf[1], outbuf[2], outbuf[3], 2,
            state);

    ctr0 = PQCLEAN_DILITHIUM4_AVX2_rej_eta(a0->coeffs, N, outbuf[0], 2 * SHAKE128_RATE);
    ctr1 = PQCLEAN_DILITHIUM4_AVX2_rej_eta(a1->coeffs, N, outbuf[1], 2 * SHAKE128_RATE);
    ctr2 = PQCLEAN_DILITHIUM4_AVX2_rej_eta(a2->coeffs, N, outbuf[2], 2 * SHAKE128_RATE);
    ctr3 = PQCLEAN_DILITHIUM4_AVX2_rej_eta(a3->coeffs, N, outbuf[3], 2 * SHAKE128_RATE);
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_poly_uniform_gamma1m1
*
* Description: Sample polynomial with uniformly random coefficients
...
void PQCLEAN_DILITHIUM4_AVX2_poly_uniform_gamma1m1(
    poly *a,
    ...
    ctr = PQCLEAN_DILITHIUM4_AVX2_rej_gamma1m1(a->coeffs, N, buf, POLY_UNIFORM_GAMMA1M1_BUFLEN);
...
void PQCLEAN_DILITHIUM4_AVX2_poly_uniform_gamma1m1_4x(poly *a0,
    ...
    PQCLEAN_DILITHIUM4_AVX2_shake256_absorb4x(state, inbuf[0], inbuf[1], inbuf[2], inbuf[3],
            CRHBYTES + 2);
    PQCLEAN_DILITHIUM4_AVX2_shake256_squeezeblocks4x(outbuf[0], outbuf[1], outbuf[2], outbuf[3], 5,
            state);
    ...
    while (ctr0 < N || ctr1 < N || ctr2 < N || ctr3 < N) {
        PQCLEAN_DILITHIUM4_AVX2_shake256_squeezeblocks4x(outbuf[0], outbuf[1], outbuf[2], outbuf[3], 1,
                state);
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyeta_pack
*
...
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyeta_pack(uint8_t *restrict r, const poly *restrict a) {
    uint8_t t[8];
    for (size_t i = 0; i < N / 8; ++i) {
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyeta_unpack
*
...
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyeta_unpack(poly *restrict r, const uint8_t *restrict a) {
    for (size_t i = 0; i < N / 8; ++i) {
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyt1_pack
*
...
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyt1_pack(uint8_t *restrict r, const poly *restrict a) {
    for (size_t i = 0; i < N / 8; ++i) {
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyt1_unpack
*
...
*              - const uint8_t *a: byte array with bit-packed polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyt1_unpack(poly *restrict r, const uint8_t *restrict a) {
    for (size_t i = 0; i < N / 8; ++i) {
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyt0_pack
*
...
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyt0_pack(uint8_t *restrict r, const poly *restrict a) {
    uint32_t t[4];
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyt0_unpack
*
...
*              - const uint8_t *a: byte array with bit-packed polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyt0_unpack(poly *restrict r, const uint8_t *restrict a) {
    for (size_t i = 0; i < N / 4; ++i) {
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyz_pack
*
...
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyz_pack(uint8_t *restrict r, const poly *restrict a) {
    uint32_t t[2];
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyz_unpack
*
...
*              - const uint8_t *a: byte array with bit-packed polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyz_unpack(poly *restrict r, const uint8_t *restrict a) {
    for (size_t i = 0; i < N / 2; ++i) {
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM4_AVX2_polyw1_pack
*
...
*              - const poly *a: pointer to input polynomial
**************************************************/
void PQCLEAN_DILITHIUM4_AVX2_polyw1_pack(
    uint8_t *restrict r,
    const poly *restrict a) {
    for (size_t i = 0; i < N / 2; ++i) {
