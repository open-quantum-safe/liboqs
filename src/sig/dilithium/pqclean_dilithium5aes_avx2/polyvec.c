...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvec_matrix_expand(polyvecl mat[K], const uint8_t rho[SEEDBYTES]) {
    ...
    PQCLEAN_DILITHIUM5AES_AVX2_aes256ctr_init(&state, rho, 0);

    for (i = 0; i < K; i++) {
        for (j = 0; j < L; j++) {
            nonce = (i << 8) + j;
            state.n = _mm_loadl_epi64((__m128i *)&nonce);
            PQCLEAN_DILITHIUM5AES_AVX2_poly_uniform_preinit(&mat[i].vec[j], &state);
            PQCLEAN_DILITHIUM5AES_AVX2_poly_nttunpack(&mat[i].vec[j]);
        }
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvec_matrix_pointwise_montgomery(polyveck *t, const polyvecl mat[K], const polyvecl *v) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_pointwise_acc_montgomery(&t->vec[i], &mat[i], v);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_uniform_eta(polyvecl *v, const uint8_t seed[SEEDBYTES], uint16_t nonce) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_uniform_eta(&v->vec[i], seed, nonce++);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_uniform_gamma1(polyvecl *v, const uint8_t seed[SEEDBYTES], uint16_t nonce) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_uniform_gamma1(&v->vec[i], seed, L * nonce + i);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_reduce(polyvecl *v) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_reduce(&v->vec[i]);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_add(polyvecl *w, const polyvecl *u, const polyvecl *v) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_add(&w->vec[i], &u->vec[i], &v->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_ntt
*
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_ntt(polyvecl *v) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_ntt(&v->vec[i]);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_invntt_tomont(polyvecl *v) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_invntt_tomont(&v->vec[i]);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_pointwise_poly_montgomery(polyvecl *r, const poly *a, const polyvecl *v) {
    ...
    for (i = 0; i < L; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_pointwise_montgomery(&r->vec[i], a, &v->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_pointwise_acc_montgomery
*
...
**************************************************/
void PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_pointwise_acc_montgomery(poly *w, const polyvecl *u, const polyvecl *v) {
    PQCLEAN_DILITHIUM5AES_AVX2_pointwise_acc_avx(w->vec, u->vec->vec, v->vec->vec, PQCLEAN_DILITHIUM5AES_AVX2_qdata.vec);
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_chknorm
*
* Description: Check infinity norm of polynomials in vector of length L.
*              Assumes input polyvecl to be reduced by PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_reduce().
*
...
* and 1 otherwise.
**************************************************/
int PQCLEAN_DILITHIUM5AES_AVX2_polyvecl_chknorm(const polyvecl *v, int32_t bound)  {
    unsigned int i;

    for (i = 0; i < L; ++i) {
        if (PQCLEAN_DILITHIUM5AES_AVX2_poly_chknorm(&v->vec[i], bound)) {
            return 1;
        }
    }
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_reduce
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_caddq
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_add
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_sub
*
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyveck_sub(polyveck *w, const polyveck *u, const polyveck *v) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_sub(&w->vec[i], &u->vec[i], &v->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_shiftl
*
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyveck_shiftl(polyveck *v) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_shiftl(&v->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_ntt
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_invntt_tomont
*
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_chknorm
*
* Description: Check infinity norm of polynomials in vector of length K.
*              Assumes input polyveck to be reduced by PQCLEAN_DILITHIUM5AES_AVX2_polyveck_reduce().
*
...
* and 1 otherwise.
**************************************************/
int PQCLEAN_DILITHIUM5AES_AVX2_polyveck_chknorm(const polyveck *v, int32_t bound) {
    unsigned int i;
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_power2round
*
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyveck_power2round(polyveck *v1, polyveck *v0, const polyveck *v) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_power2round(&v1->vec[i], &v0->vec[i], &v->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_decompose
*
* Description: For all coefficients a of polynomials in vector of length K,
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyveck_decompose(polyveck *v1, polyveck *v0, const polyveck *v) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_decompose(&v1->vec[i], &v0->vec[i], &v->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_make_hint
*
...
unsigned int PQCLEAN_DILITHIUM5AES_AVX2_polyveck_make_hint(uint8_t *hint, const polyveck *v0, const polyveck *v1) {
    ...
    for (i = 0; i < K; ++i) {
        n += PQCLEAN_DILITHIUM5AES_AVX2_poly_make_hint(&hint[n], &v0->vec[i], &v1->vec[i]);
    }
...
/*************************************************
* Name:        PQCLEAN_DILITHIUM5AES_AVX2_polyveck_use_hint
*
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyveck_use_hint(polyveck *w, const polyveck *u, const polyveck *h) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_poly_use_hint(&w->vec[i], &u->vec[i], &h->vec[i]);
    }
}
...
void PQCLEAN_DILITHIUM5AES_AVX2_polyveck_pack_w1(uint8_t r[K * POLYW1_PACKEDBYTES], const polyveck *w1) {
    ...
    for (i = 0; i < K; ++i) {
        PQCLEAN_DILITHIUM5AES_AVX2_polyw1_pack(&r[i * POLYW1_PACKEDBYTES], &w1->vec[i]);
    }
}
