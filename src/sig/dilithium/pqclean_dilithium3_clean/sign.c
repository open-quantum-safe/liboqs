...
/*************************************************
* Name:        PQCLEAN_DILITHIUM3_CLEAN_expand_mat
*
...
**************************************************/
void PQCLEAN_DILITHIUM3_CLEAN_expand_mat(polyvecl mat[K], const uint8_t rho[SEEDBYTES]) {
...
*
* Arguments:   - uint8_t *sig:  pointer to output signature (PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES
*                               of len)
*              - size_t *smlen: pointer to output length of signed message
*                               (should be PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES)
...
int PQCLEAN_DILITHIUM3_CLEAN_crypto_sign(
    ...
    const uint8_t *sk) {
    int rc;
    memmove(sm + PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES, m, mlen);
...
*              - size_t siglen: length of signature (PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES)
...
int PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_open(
    ...
    *mlen = smlen - PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES;
    ...
    if (PQCLEAN_DILITHIUM3_CLEAN_crypto_sign_verify(sm, PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES,
            sm + PQCLEAN_DILITHIUM3_CLEAN_CRYPTO_BYTES, *mlen, pk)) {
        goto badsig;
    } else {
