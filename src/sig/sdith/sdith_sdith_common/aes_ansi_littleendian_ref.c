 ...
 *
 * Little-endian reworking of aes_ansi_ref.c (the optimised ANSI C Rijndael code
 * by Vincent Rijmen, Antoon Bosselaers and Paulo Barreto -- public domain).
 *
 * The original reference stores each 32-bit round-key / state word in BIG-ENDIAN
 * byte order (byte 0 of a column lands in the most-significant byte of the u32).
 * On a little-endian machine that byte-swaps every word when it hits memory, so
 * the expanded round keys produced by rijndaelKeySetupEnc do NOT match the raw
 * 16-byte little-endian layout that the AES-NI / AVX2 path uses
 * (_mm_loadu_si128(key) ... _mm_storeu_si128(rk)).
 *
 * This file keeps the exact same algorithm but works entirely in LITTLE-ENDIAN
 * word order (byte 0 of a column lives in the least-significant byte of the u32).
 * As a result:
 *   - GETU32 / PUTU32 are plain little-endian load / store (no byte swap);
 *   - rk[0] = key bytes [0..3], so the round-key buffer is byte-for-byte identical
 *     to the AVX2 key schedule (aes128_ctrle_set_key_avx2) -- a 1-1 mapping.
 * The T-tables below are the little-endian (byte-reversed) counterparts of the
 * original Te0..Te3; Te4 (S-box replicated in all 4 bytes) is byte-symmetric and
 * therefore unchanged. Verified against the FIPS-197 128/192/256 test vectors.
 *
 * This code is hereby placed in the public domain.
 *
 * THIS SOFTWARE IS PROVIDED BY THE AUTHORS ''AS IS'' AND ANY EXPRESS
 * OR IMPLIED WARRANTIES ARE DISCLAIMED (see aes_ansi_ref.c for the full notice).
 */
...
/*
 * Little-endian T-tables. Byte i (i = LSB..MSB) of Tej[x] holds the MixColumns
 * contribution of S[x] to output row i, when x is the input byte of row j:
 *   Te0[x] = [02, 01, 01, 03].S[x]   (row-0 input)
 *   Te1[x] = [03, 02, 01, 01].S[x]   (row-1 input)
 *   Te2[x] = [01, 03, 02, 01].S[x]   (row-2 input)
 *   Te3[x] = [01, 01, 03, 02].S[x]   (row-3 input)
 *   Te4[x] = [01, 01, 01, 01].S[x]   (pure SubBytes, used by key setup + last round)
 */
...
/* Te4[x] = S-box value replicated in all four bytes (byte-symmetric, unchanged). */
...
/* rcon injected into the least-significant byte (byte 0) of the little-endian word. */
static const u32 rcon[] = {
    0x00000001, 0x00000002, 0x00000004, 0x00000008, 0x00000010, 0x00000020,
    0x00000040, 0x00000080, 0x0000001b, 0x00000036, /* 128-bit blocks never need more than 10 rcon values */
};

/* little-endian load / store: no byte swap, so rk[]/state words match the raw
 * 16-byte layout used by the AES-NI / AVX2 path. */
 ...
 *
 * The round keys are stored in little-endian word order, so the resulting buffer
 * is byte-for-byte identical to the AES-NI / AVX2 key schedule.
 *
...
int rijndaelKeySetupEnc(u32 rk[/*4*(Nr + 1)*/], const u8 cipherKey[], int keyBits) {
  ...
  if (keyBits == 128) {
    for (;;) {
      temp = rk[3];
      /* SubWord(RotWord(temp)) ^ rcon, little-endian: byte0=S[b1], byte1=S[b2], byte2=S[b3], byte3=S[b0]. */
      rk[4] = rk[0] ^ (Te4[(temp >> 8) & 0xff] & 0x000000ff) ^ (Te4[(temp >> 16) & 0xff] & 0x0000ff00) ^
              (Te4[(temp >> 24) & 0xff] & 0x00ff0000) ^ (Te4[(temp) & 0xff] & 0xff000000) ^ rcon[i];
      rk[5] = rk[1] ^ rk[4];
      rk[6] = rk[2] ^ rk[5];
      rk[7] = rk[3] ^ rk[6];
      if (++i == 10) {
  ...
  if (keyBits == 192) {
    for (;;) {
      temp = rk[5];
      rk[6] = rk[0] ^ (Te4[(temp >> 8) & 0xff] & 0x000000ff) ^ (Te4[(temp >> 16) & 0xff] & 0x0000ff00) ^
              (Te4[(temp >> 24) & 0xff] & 0x00ff0000) ^ (Te4[(temp) & 0xff] & 0xff000000) ^ rcon[i];
      rk[7] = rk[1] ^ rk[6];
      rk[8] = rk[2] ^ rk[7];
      rk[9] = rk[3] ^ rk[8];
  ...
  if (keyBits == 256) {
    for (;;) {
      temp = rk[7];
      rk[8] = rk[0] ^ (Te4[(temp >> 8) & 0xff] & 0x000000ff) ^ (Te4[(temp >> 16) & 0xff] & 0x0000ff00) ^
              (Te4[(temp >> 24) & 0xff] & 0x00ff0000) ^ (Te4[(temp) & 0xff] & 0xff000000) ^ rcon[i];
      rk[9] = rk[1] ^ rk[8];
      rk[10] = rk[2] ^ rk[9];
      rk[11] = rk[3] ^ rk[10];
      ...
      temp = rk[11];
      /* SubWord(temp) with no rotate, little-endian: S applied in place to each byte. */
      rk[12] = rk[4] ^ (Te4[(temp) & 0xff] & 0x000000ff) ^ (Te4[(temp >> 8) & 0xff] & 0x0000ff00) ^
               (Te4[(temp >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(temp >> 24) & 0xff] & 0xff000000);
      rk[13] = rk[5] ^ rk[12];
      rk[14] = rk[6] ^ rk[13];
      rk[15] = rk[7] ^ rk[14];
...
void rijndaelEncrypt(const u32 rk[/*4*(Nr + 1)*/], int Nr, const u8 pt[16], u8 ct[16]) {
  ...
  /*
   * map byte array block to cipher state (little-endian words)
  ...
  for (;;) {
    ...
    s3 = Te0[t3 & 0xff] ^ Te1[(t0 >> 8) & 0xff] ^ Te2[(t1 >> 16) & 0xff] ^ Te3[t2 >> 24] ^ rk[3];
        ...
        * map cipher state to byte array block (little-endian):
        */
  s0 = (Te4[(t0) & 0xff] & 0x000000ff) ^ (Te4[(t1 >> 8) & 0xff] & 0x0000ff00) ^ (Te4[(t2 >> 16) & 0xff] & 0x00ff0000) ^
       (Te4[(t3 >> 24)] & 0xff000000) ^ rk[0];
  PUTU32(ct, s0);
  s1 = (Te4[(t1) & 0xff] & 0x000000ff) ^ (Te4[(t2 >> 8) & 0xff] & 0x0000ff00) ^ (Te4[(t3 >> 16) & 0xff] & 0x00ff0000) ^
       (Te4[(t0 >> 24)] & 0xff000000) ^ rk[1];
  PUTU32(ct + 4, s1);
  s2 = (Te4[(t2) & 0xff] & 0x000000ff) ^ (Te4[(t3 >> 8) & 0xff] & 0x0000ff00) ^ (Te4[(t0 >> 16) & 0xff] & 0x00ff0000) ^
       (Te4[(t1 >> 24)] & 0xff000000) ^ rk[2];
  PUTU32(ct + 8, s2);
  s3 = (Te4[(t3) & 0xff] & 0x000000ff) ^ (Te4[(t0 >> 8) & 0xff] & 0x0000ff00) ^ (Te4[(t1 >> 16) & 0xff] & 0x00ff0000) ^
       (Te4[(t2 >> 24)] & 0xff000000) ^ rk[3];
  PUTU32(ct + 12, s3);
}

#include "aes_ansi_ref.h"

EXPORT void aes128_set_key_ref(void* rk, const void* key128) { rijndaelKeySetupEnc(rk, key128, 128); }
