...
OQS_SIG *OQS_SIG_faest_em_256s_new(void) {
	...
	sig->keypair = OQS_SIG_faest_em_256s_keypair;
	sig->sign = OQS_SIG_faest_em_256s_sign;
	sig->verify = OQS_SIG_faest_em_256s_verify;
	sig->sign_with_ctx_str = OQS_SIG_faest_em_256s_sign_with_ctx_str;
	sig->verify_with_ctx_str = OQS_SIG_faest_em_256s_verify_with_ctx_str;

	return sig;
}

extern int PQCLEAN_FAEST_EM_256S_REF_crypto_sign_keypair(uint8_t *pk, uint8_t *sk);
extern int PQCLEAN_FAEST_EM_256S_REF_crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk);
extern int PQCLEAN_FAEST_EM_256S_REF_crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m, size_t mlen, const uint8_t *pk);

OQS_API OQS_STATUS OQS_SIG_faest_em_256s_keypair(uint8_t *public_key, uint8_t *secret_key) {
	return (OQS_STATUS) PQCLEAN_FAEST_EM_256S_REF_crypto_sign_keypair(public_key, secret_key);
}
...
OQS_API OQS_STATUS OQS_SIG_faest_em_256s_sign(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *secret_key) {
	return (OQS_STATUS) PQCLEAN_FAEST_EM_256S_REF_crypto_sign_signature(signature, signature_len, message, message_len, secret_key);
}
...
OQS_API OQS_STATUS OQS_SIG_faest_em_256s_verify(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *public_key) {
	return (OQS_STATUS) PQCLEAN_FAEST_EM_256S_REF_crypto_sign_verify(signature, signature_len, message, message_len, public_key);
}
