...
__inline void
oqs_sidh_cln16_fpadd751(digit_t *a, digit_t *b,
...
__inline void
oqs_sidh_cln16_fpsub751(digit_t *a, digit_t *b,
...
void oqs_sidh_cln16_fpdiv2_751(
    digit_t *a, digit_t *c) { // Modular division by two, c = a/2 mod p751.
...
void oqs_sidh_cln16_mp_mul_schoolbook(
    digit_t *a, digit_t *b, digit_t *c,
    unsigned int nwords) { // Multiprecision schoolbook multiply, c = a*b, where
                           // lng(a) = lng(b) = nwords.
  unsigned int i, j;
  digit_t u, v, UV[2];
...
void oqs_sidh_cln16_mp_mul_comba(digit_t *a, digit_t *b, digit_t *c,
                                 unsigned int nwords) { // Multiprecision comba
                                                        // multiply, c = a*b,
                                                        // where lng(a) = lng(b)
                                                        // = nwords.
  unsigned int i, j;
...
void oqs_sidh_cln16_rdc_mont(oqs_sidh_cln16_dfelm_t ma,
                             oqs_sidh_cln16_felm_t mc) { // Optimized Montgomery
                                                         // reduction using
