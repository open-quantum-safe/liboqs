...
void oqs_sidh_cln16_j_inv(oqs_sidh_cln16_f2elm_t A, oqs_sidh_cln16_f2elm_t C,
  ...
  // Input: A,C in GF(p^2).
  // Output: j=256*(A^2-3*C^2)^3/(C^4*(A^2-4*C^2)), which is j-invariant of
  // Montgomery curve B*y^2=x^3+(A/C)*x^2+x or (equivalently) j-invariant of
...
void oqs_sidh_cln16_xDBL_basefield(oqs_sidh_cln16_point_basefield_proj_t P,
  ...
  // Doubling of a Montgomery point in projective coordinates (X:Z) over the
  // base field.
  // Input: projective Montgomery x-coordinates P = (X1:Z1), where x1=X1/Z1 and
  // Montgomery curve constant A24/C24=(A/C+2)/4.
  // Output: projective Montgomery x-coordinates Q = 2*P = (X2:Z2).
...
void oqs_sidh_cln16_xDBLADD_basefield(oqs_sidh_cln16_point_basefield_proj_t P,
  ...
  oqs_sidh_cln16_fpmul751_mont(P->Z, t2, P->Z); // ZP =
...
oqs_sidh_cln16_BigMont_ladder(unsigned char *x, digit_t *m, unsigned char *xout,
  ...
  oqs_sidh_cln16_ladder(X, scalar, P1, P2, A24, BIGMONT_NBITS_ORDER,
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_secret_pt(oqs_sidh_cln16_point_basefield_t P,
  ...
  // 1-dimensional Montgomery ladder in the trace zero subgroup and
  ...
  // oqs_sidh_cln16_xDBLADD_basefield() in the ladder computation
  A24[0] = 1;
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_ladder_3_pt(
  ...
  oqs_sidh_cln16_point_proj_t U = oqs_sidh_cln16_point_proj_t_EMPTY,
                              V = oqs_sidh_cln16_point_proj_t_EMPTY;
...
void oqs_sidh_cln16_get_4_isog(oqs_sidh_cln16_point_proj_t P,
  ...
  // Computes the corresponding 4-isogeny of a projective Montgomery point
  // (X4:Z4) of order 4.
  // Input:  projective point of order four P = (X4:Z4).
  // Output: the 4-isogenous Montgomery curve with projective coefficient A/C
  // and the 5 coefficients
  //         that are used to evaluate the isogeny at a point in
  //         oqs_sidh_cln16_eval_4_isog().
...
void oqs_sidh_cln16_get_3_isog(oqs_sidh_cln16_point_proj_t P,
                               oqs_sidh_cln16_f2elm_t A,
                               oqs_sidh_cln16_f2elm_t C) {
  // Computes the corresponding 3-isogeny of a projective Montgomery point
  // (X3:Z3) of order 3.
...
void oqs_sidh_cln16_eval_3_isog(oqs_sidh_cln16_point_proj_t P,
                                oqs_sidh_cln16_point_proj_t Q) {
  // Computes the 3-isogeny R=phi(X:Z), given projective point (X3:Z3) of order
  // 3 on a Montgomery curve and a point P = (X:Z).
...
void oqs_sidh_cln16_get_A(oqs_sidh_cln16_f2elm_t xP, oqs_sidh_cln16_f2elm_t xQ,
                          ...
                          PCurveIsogenyStruct CurveIsogeny) {
  // Given the x-coordinates of P, Q, and R, returns the value A corresponding
  // to the Montgomery curve E_A: y^2=x^3+A*x^2+x such that R=Q-P on E_A.
