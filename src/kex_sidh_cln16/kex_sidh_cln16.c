...
OQS_KEX *OQS_KEX_sidh_cln16_new(OQS_RAND *rand) {
  ...
  if (oqs_sidh_cln16_curve_initialize(
          curveIsogeny, rand, &CurveIsogeny_SIDHp751) != SIDH_CRYPTO_SUCCESS) {
    free(k);
  ...
  k->seed_len = 0;
  k->named_parameters =
      NULL; // TODO: create param p751 when we have more curves
  k->rand = rand;
  k->params = NULL;
...
int OQS_KEX_sidh_cln16_alice_0(OQS_KEX *k, void **alice_priv,
  ...
  *alice_priv = malloc(SIDH_SECRETKEY_LEN);
  ...
  if (oqs_sidh_cln16_KeyGeneration_A((unsigned char *)*alice_priv,
...
int OQS_KEX_sidh_cln16_bob(OQS_KEX *k, const uint8_t *alice_msg,
  ...
  *key = malloc(SIDH_SHAREDKEY_LEN);
  ...
  if (oqs_sidh_cln16_KeyGeneration_B((unsigned char *)bob_priv,
                                     (unsigned char *)*bob_msg, k->ctx,
                                     k->rand) != SIDH_CRYPTO_SUCCESS) {
    goto err;
  }
  if (oqs_sidh_cln16_SecretAgreement_B(
          (unsigned char *)bob_priv, (unsigned char *)alice_msg,
          (unsigned char *)*key, 0, k->ctx, k->rand) != SIDH_CRYPTO_SUCCESS) {
    goto err;
  }
...
int OQS_KEX_sidh_cln16_alice_1(OQS_KEX *k, const void *alice_priv,
  ...
  *key = malloc(SIDH_SHAREDKEY_LEN);
  ...
  if (oqs_sidh_cln16_SecretAgreement_A((unsigned char *)alice_priv,
                                       (unsigned char *)bob_msg,
                                       (unsigned char *)*key, false, k->ctx,
                                       k->rand) != SIDH_CRYPTO_SUCCESS) {
    goto err;
  }
