...
*
* SECURITY NOTE: these functions run in variable time because it is assumed that
*they are
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_random_fp2(oqs_sidh_cln16_f2elm_t f2value,
  ...
  nbytes =
      (pCurveIsogeny->pbits + 7) / 8; // Number of random bytes to be requested
  ...
  do {
    ntry++;
    if (ntry >
        100) { // Max. 100 iterations to obtain random value in [0, p751-1]
...
static bool test_curve(oqs_sidh_cln16_f2elm_t A, oqs_sidh_cln16_f2elm_t rvalue,
                       PCurveIsogenyStruct CurveIsogeny) {
  // This function checks that the curve is in the correct supersingular isogeny
  // class via Sutherland's Monte Carlo algorithm.
  // It also checks that the curve is not a subfield curve. Both Alice and Bob
  // call this same function in their respective validation procedures below.
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_Validate_PKA(oqs_sidh_cln16_f2elm_t A,
  ...
  // Inputs: Alice's public key [A,xP,xQ,xQP], where xP,xQ and xQP are contained
  // in PKA,
  ...
  oqs_sidh_cln16_point_proj_t P = oqs_sidh_cln16_point_proj_t_EMPTY,
                              Q = oqs_sidh_cln16_point_proj_t_EMPTY;
  SIDH_CRYPTO_STATUS Status;

  // Choose a random element in GF(p751^2) for Sutherland's algorithm. Assume
  // that it is in Montgomery representation
  ...
  *valid = !is_equal_fp2(t0, zero); // Checks that ZP*ZQ*(XQ*ZP-XP*ZQ) != 0,
                                    // i.e., that 3^(e-1)*P != 3^(e-1)*Q and
                                    // neither P nor Q has order 3^(e-1)
  ...
  *valid = *valid &
           test_curve(A, rvalue,
                      CurveIsogeny); // Tests curve via Sutherland's algorithm
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_Validate_PKB(oqs_sidh_cln16_f2elm_t A,
  ...
  // Inputs: Bob's public key [A,xP,xQ,xQP], where xP,xQ and xQP are contained
  // in PKB,
  ...
  oqs_sidh_cln16_fp2correction751(t0);
  *valid = !is_equal_fp2(t0, zero); // Checks that ZP*ZQ*(XQ*ZP-XP*ZQ) != 0,
                                    // i.e., that 2^(e-2)*P != 2^(e-2)*Q and
                                    // neither P nor Q has order 2^(e-2)
  ...
  *valid =
      *valid &
      !is_equal_fp2(t0, zero); // Checks that 2^(e-1)*P != 0 and 2^(e-1)*Q != 0
