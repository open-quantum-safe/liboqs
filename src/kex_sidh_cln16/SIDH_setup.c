...
oqs_sidh_cln16_curve_allocate(PCurveIsogenyStaticData CurveData) {
  ...
  PCurveIsogenyStruct pCurveIsogeny =
      (PCurveIsogenyStruct)calloc(1, sizeof(CurveIsogenyStruct));
  if (!pCurveIsogeny) {
    return NULL;
  }
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_random_BigMont_mod_order(
  ...
  // If successful, the output is given in "random_digits" in the range [1,
  // BigMont_order-1].
  unsigned int ntry = 0, nbytes = (BIGMONT_NBITS_ORDER + 7) / 8,
               nwords = NBITS_TO_NWORDS(BIGMONT_NBITS_ORDER);
