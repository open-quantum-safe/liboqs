...
static void EncryptAES_ECB(const byte *msg, size_t msg_len, const byte *key, byte *c) {
    assert(msg_len % 16 == 0);
    for (size_t block = 0; block < msg_len / 16; block++) {
        EncryptAES(msg + (16 * block), key, c + (16 * block));
    }
}
