...
static void AES192_ECB_load_schedule(const uint8_t *key, void **_schedule) {
	C_OR_NI_OR_ARM(
	    oqs_aes192_load_schedule_c(key, _schedule),
	    oqs_aes192_load_schedule_ni(key, _schedule),
	    oqs_aes192_load_schedule_no_bitslice(key, _schedule)
	);
}

static void AES192_free_schedule(void *schedule) {
	C_OR_NI_OR_ARM(
	    oqs_aes192_free_schedule_c(schedule),
	    oqs_aes192_free_schedule_ni(schedule),
	    oqs_aes192_free_schedule_no_bitslice(schedule)
	);
}

static void AES192_ECB_enc_sch(const uint8_t *plaintext, const size_t plaintext_len, const void *schedule, uint8_t *ciphertext);
...
static void AES192_ECB_enc_sch(const uint8_t *plaintext, const size_t plaintext_len, const void *schedule, uint8_t *ciphertext) {
	C_OR_NI_OR_ARM(
	    oqs_aes192_ecb_enc_sch_c(plaintext, plaintext_len, schedule, ciphertext),
	    oqs_aes192_ecb_enc_sch_ni(plaintext, plaintext_len, schedule, ciphertext),
	    oqs_aes192_ecb_enc_sch_armv8(plaintext, plaintext_len, schedule, ciphertext)
	);
}
...
struct OQS_AES_callbacks aes_default_callbacks = {
	...
	.AES192_ECB_load_schedule = AES192_ECB_load_schedule,
