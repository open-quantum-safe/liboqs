 ...
 * @param ctx            Abstract data structure for a key schedule.
 */
void OQS_AES256_CTR_load_schedule(const uint8_t *key, void **ctx);

/**
 * Function to fill a context given a nonce for use in CTR mode.
 *
 ...
 * @param ctx            Abstract data structure for nonce
 */
void OQS_AES256_CTR_load_nonce(const uint8_t *iv, size_t iv_len, void **ctx);
 ...
 */
void OQS_AES256_CTR_sch(const uint8_t *iv, size_t iv_len, const void *ctx, uint8_t *out, size_t out_len);

/**
 * AES counter mode keystream generator. A context generated by
 * OQS_AES256_CTR_load_schedule() and OQS_AES256_CTR_load_nonce() is passed
 * rather than a key and an IV.
 *
 * @param ctx      Abstract data structure for key schedule and IV.
 * @param out      Pointer to a block of memory which is big enough to contain out_len bytes; the result will be written here.
 * @param out_len  Length of output bytes to generate.
 */
void OQS_AES256_CTR_sch_ivinit(void *ctx, uint8_t *out, size_t out_len);
