...
#define aes256ctr_squeezeblocks(out, outlen, state) OQS_AES256_CTR_sch_upd_blks(*state, out, 4*outlen)
#define aes256ctr_init_key(state, key) OQS_AES256_CTR_load_schedule(key, state)
#define aes256ctr_init_iv(state, nonce) OQS_AES256_CTR_load_iv(nonce, 12, *state)

static inline void aes256ctr_init(void **_schedule, const uint8_t *key, const uint8_t *nonce) {
	aes256ctr_init_key(_schedule, key);
	aes256ctr_init_iv(_schedule, nonce);
}
