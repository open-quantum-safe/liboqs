...
Integrating external implementations into liboqs
------------------------------------------------

All key exchange algorithms are accessed through a common API described under src/kex.h.
Concrete key exchange implementations are called from a wrapper, src/kex.c, implementing the API.
Concrete key exchange implementations implement the API under src/kex.h.
Public interface methods start with a prefix *OQS_KEX_*.

If a method is not exposed publicly then it should be declared static or else global namespace 
pollution error will be flagged causing the build to build.

It is recommended that .travis-tests.sh is run before pushing with the concerned algorithm enabled.
The reason for doing this is to make sure all the tests and checks under .travis are covered. 

The goal is to not modify the original external implementation, instead apply patch(s)
which such the patched code satisfies all the requirements mentioned above and builds without warnings.

For an example please look into the following directory under liboqs/:

*patches/kex_rlwe_newhope_avx2*

### Non-patch related changes for integration

configure.ac and Makefile.am at the top level have to be updated to include the new algorithm.
Please consult those files to see how new algorithms are integrated.

style-check.sh under .travis/ needs an update to skip original code's formatting.
Please consult the file for an example.

### Creation of patch

1. Original source code is checked in and pushed.

2. Make all the changes required to create atleast two patches:
    
    patch0.patch contains Makefile.am
    patch1.patch contains all the changes to be applied during building such that all
    the tests under .travis directory pass.

    All the patches for external key exchange algorithms reside under:
    
      patches/{key exchange algorithm name}.

    The patches are named serially:

      patch0.patch
      patch1.patch
      ...

3. After the above step update .travis-tests.sh to enable patch application
   once the algorithm is enabled.
   
   The two variables that are going to need update are:
    
    patch_dir_str - points to a directory under patches/ where the patches
    for a given external implementation are saved.
    
    recover_from_patch_dirs_str - points to where the original source
    code is located

    If the algorithm is enabled then 
      
      ./patches/apply-patch ${patch_dir_str} is called

    else

      ./patches/apply-patch ${patch_dir_str} 0 is called



    apply-patch script applies the patch.
    
    cleanup-patch script brings back the `git status` to what it was befor the
    application of the patch. 


    Patch application and cleanup scripts are called from .travis-tests.sh
    
    The reason to have Makefile.am as a separate patch, i.e. patch0.patch is that 
    this patch has to be applied irrespective of whether the algorithm is enabled or
    not. 
    autoconf based builds require the presence of Makefile.am to generate the
    Makefile, whether the Makefile gets called or not depends on whether the
    algorithm is enabled or not.
