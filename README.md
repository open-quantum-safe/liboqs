...
- `oqs_bench_kex`: A benchmarking tool for comparing the performance of various
  KEX algorithms. 
...
Assuming that the clang toolchain is installed, do the following to run static analyzer:

  scan-build ./configure
  scan-build -o <dir to output html files> make

Once the html files are generated in the above supplied directory, index.html
can be opened to start looking into the list of bug(s)/false positives generated by static
analyzer.

IMPORTANT:

Its very important to run static analyzer before checking in the code to the
repository.

Code Coverage
-------------

Please make sure gcov and lcov are installed before running check_coverage
script.

check_coverage script under topmost folder generates coverage report.
After running the script a bunch of html files are generated under the
directory lcov-output.
index.html under lcov-output can be opened to view the coverage stats.
