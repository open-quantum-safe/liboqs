...
def derive_key_iv_hp(
    ...
    print("in QUIC crypto: Class Derive key: Algorithm being used")
    print(algorithm)
    if cipher_suite in [
        ...
        key_size = 32
        print("in QUIC crypto: Class Derive key")
        print("Keysize = 32")
    else:
        key_size = 16
        print("in QUIC crypto: Class Derive key")
...
class CryptoContext:
    def __init__(self, key_phase: int = 0) -> None:
        ...
        print("in QUIC crypto: class CryptoContext init")
    ...
    def decrypt_packet(
        ...
        # header protection
        print("in QUIC crypto: Class CryptoContext header protection")
        plain_header, packet_number = self.hp.remove(packet, encrypted_offset)
        first_byte = plain_header[0]

        # packet number
        print("in QUIC crypto: Class CrytpoContext packet number")
        ...
        print("in QUIC crypto: Class CrytpoContext detecting key phase change")
        crypto = self
        ...
        # payload protection
        print("in QUIC crypto: Class CrytpoContext payload protection")
    ...
    def encrypt_packet(
        ...
        print("in QUIC crypto: Class CrytpoContext encrypt packet ")
        assert self.is_valid(), "Encryption key is not available"

        # payload protection
        print("in QUIC crypto: Class CrytpoContext encrypt packet payload protection")
        protected_payload = self.aead.encrypt(
            plain_payload, plain_header, packet_number
        )

        # header protection
        print("in QUIC crypto: Class CrytpoContext encrypt packet header protection")
    ...
    def setup(self, cipher_suite: CipherSuite, secret: bytes, version: int) -> None:
        print("in QUIC crypto: Class CrytpoContext setup")
...
def apply_key_phase(self: CryptoContext, crypto: CryptoContext) -> None:
    print("in QUIC crypto: Class CrytpoContext apply key phase")
...
def next_key_phase(self: CryptoContext) -> CryptoContext:
    print("in QUIC crypto: Class CrytpoContext next key phase")

    algorithm = cipher_suite_hash(self.cipher_suite)
    print("next key phase algorythm")
    print(algorithm)
...
class CryptoPair:
    def __init__(self) -> None:
        ...
        print("in QUIC crypto: Class CrytpoPair init")


    def decrypt_packet(
        self, packet: bytes, encrypted_offset: int, expected_packet_number: int
    ) -> Tuple[bytes, bytes, int]:
        print("in QUIC crypto: Class CrytpoPair decrypt packet")
    ...
    def encrypt_packet(
        ...
        print("in QUIC crypto: Class CrytpoPair encrypt packet")
    ...
    def setup_initial(self, cid: bytes, is_client: bool, version: int) -> None:
        print("in QUIC crypto: Class CrytpoPair setup initial")
        if is_client:
        ...
        print("in QUIC crypto: Class CrytpoPair setup initial")
        print("Algorythm used:")
        print(INITIAL_CIPHER_SUITE)
    ...
    def update_key(self) -> None:
        print("in QUIC crypto: Class CrytpoPair update key")
