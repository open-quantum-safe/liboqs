...
def load_pem_x509_certificates(data: bytes) -> List[x509.Certificate]:
    ...
    """
    print ("In TLS: class State load_pem_x509_certificates")
...
def openssl_assert(ok: bool, func: str) -> None:
    print ("In TLS: class State openssl assert")
    if not ok:
        lib.ERR_clear_error()
...
def cert_x509_ptr(certificate: x509.Certificate) -> Any:
    print ("In TLS: class State cert x509 ptr")
...
def verify_certificate(
    ...
    print ("In TLS: class State verify certificate date")
    # verify dates
    now = utcnow()
    ...
    print ("In TLS: class State verify certificate chain")
    store = lib.X509_STORE_new()
    openssl_assert(store != ffi.NULL, "X509_store_new")
    store = ffi.gc(store, lib.X509_STORE_free)

    # load default CAs
    print ("In TLS: class State verify certificate load default CAs")
    openssl_assert(
    ...
    # load extra CAs
    print ("In TLS: class State verify certificate load extra CAs")
...
def pull_client_hello(buf: Buffer) -> ClientHello:
    print ("In TLS: class State verify certificate pull client Hello")
    ...
    with pull_block(buf, 3):
        ...
        def pull_extension() -> None:
            ...
            if extension_type == ExtensionType.KEY_SHARE:
                ...
                print ("Printing hello.signature algorithm in pull client")
                print (hello.signature_algorithms)
...
def push_client_hello(buf: Buffer, hello: ClientHello) -> None:
    print ("In TLS: class State verify certificate push client Hello")
...
def pull_server_hello(buf: Buffer) -> ServerHello:
    print ("In TLS: class State verify certificate pull server Hello")
...
def push_server_hello(buf: Buffer, hello: ServerHello) -> None:
    print ("In TLS: class State verify certificate load push server Hello")
    ...
    with push_block(buf, 3):
        ...
        with push_block(buf, 2):
            ...
            if hello.key_share is not None:
                with push_extension(buf, ExtensionType.KEY_SHARE):
                    print ("In push server hello printing server key shre")
                    print (hello.key_share)
...
def pull_new_session_ticket(buf: Buffer) -> NewSessionTicket:
    print ("In TLS: class State verify certificate pull new session ticket")
...
def push_new_session_ticket(buf: Buffer, new_session_ticket: NewSessionTicket) -> None:
    print ("In TLS: class State verify certificate push new session ticket")
...
def pull_encrypted_extensions(buf: Buffer) -> EncryptedExtensions:
    print ("In TLS: class State verify certificate pull encrypted extentions")
...
def push_encrypted_extensions(buf: Buffer, extensions: EncryptedExtensions) -> None:
    print ("In TLS: class State verify certificate push encrypted extentions")
...
def pull_certificate(buf: Buffer) -> Certificate:
    print ("In TLS: class State verify certificate pull certificate")
...
def push_certificate(buf: Buffer, certificate: Certificate) -> None:
    print ("In TLS: class State verify certificate push certificate")
...
def pull_certificate_verify(buf: Buffer) -> CertificateVerify:
    print ("In TLS: class State verify certificate pull certificate verify")
    assert buf.pull_uint8() == HandshakeType.CERTIFICATE_VERIFY
    with pull_block(buf, 3):
        algorithm = buf.pull_uint16()
        signature = pull_opaque(buf, 2)

        print ("in pull cert verify printing algorythm")
        print (algorithm)
...
def push_certificate_verify(buf: Buffer, verify: CertificateVerify) -> None:
    print ("In TLS: class State verify certificate push certificate verify")
...
class KeySchedule:
    ...
    def derive_secret(self, label: bytes) -> bytes:
        print ("In TLS: class KeySchedule derive secret")
...
def decode_public_key(
    ...
    print ("In TLS: class KeySchedule decode public key")
    if key_share[0] == Group.X25519:
        print ("In TLS: class KeySchedule decode public key X25519PublicKey")
        return x25519.X25519PublicKey.from_public_bytes(key_share[1])
    elif key_share[0] == Group.X448:
        print ("In TLS: class KeySchedule decode public key X448PublicKey")
        return x448.X448PublicKey.from_public_bytes(key_share[1])
    elif key_share[0] in GROUP_TO_CURVE:
        print ("In TLS: class KeySchedule decode public key EllipticCurve")
...
def encode_public_key(
    ...
    print ("In TLS: class KeySchedule encode public key")
    if isinstance(public_key, x25519.X25519PublicKey):
        print ("In TLS: class KeySchedule encode public key X25519PublicKey")
        return (Group.X25519, public_key.public_bytes(Encoding.Raw, PublicFormat.Raw))
    elif isinstance(public_key, x448.X448PublicKey):
        print ("In TLS: class KeySchedule encode public key X448PublicKey")
...
def negotiate(
    ...
    print ("In TLS: class KeySchedule negotiate")
...
def signature_algorithm_params(
    ...
    print ("In TLS: class KeySchedule Signature algorithm papams")
...
def push_message(
    ...
    print ("In TLS: class KeySchedule push message")
...
class SessionTicket:
    ...
    """
    print ("In TLS: class SessionTicket ")
    age_add: int
...
class Context:
    def __init__(
        ...
        print ("In TLS: class Context init")
        # configuration
    ...
    def session_resumed(self) -> bool:
        ...
        """
        print ("In TLS: class Context session resumed")
        return self._session_resumed

    def handle_message(
        self, input_data: bytes, output_buf: Dict[Epoch, Buffer]
    ) -> None:
        print ("In TLS: class Context handle message")
        ...
        while len(self._receive_buffer) >= 4:
            ...
            print ("In TLS: class Context handle determine message length")
            ...
            print ("In TLS: class Context handle check message is complete")
            ...
            # client states
            print ("In TLS: class Context client states")
    ...
    def _build_session_ticket(
        ...
        print ("In TLS: class Context build session ticket")
    ...
    def _client_send_hello(self, output_buf: Buffer) -> None:
        print ("In TLS: class Context client send hello")
        key_share: List[KeyShareEntry] = []
        supported_groups: List[int] = []

        for group in self._supported_groups:
            if group == Group.SECP256R1:
                print ("In TLS: class Context client send hello using Elliptic-curve")
                ...
                print ("In TLS: class Context client send hello using x25519_private_key")
                ...
                print ("In TLS: class Context client send hello using xx448_private_key")
        ...
        if self.session_ticket and self.session_ticket.is_valid:
            ("In TLS: class Context client send hello session ticket is valid")
    ...
    def _client_handle_hello(self, input_buf: Buffer, output_buf: Buffer) -> None:
        ("In TLS: class Context client send hello client handle hello")
        ...
        # select key schedule
        ("In TLS: class Context client send hello selecting key schedule")
        ...
        ("In TLS: class Context client send hello performing key exchange")
        ...
        if (
            ...
            ("In TLS: class Context client send hello performing key exchange x5519 public key")
            ...
            ("In TLS: class Context client send hello performing key exchange x448 public key")
            ...
            ("In TLS: class Context client send hello performing key exchange Elliptic curve public key")
    ...
    def _client_handle_encrypted_extensions(self, input_buf: Buffer) -> None:
        print ("In TLS: class Context client client handle encrypted extentions")
    ...
    def _client_handle_certificate(self, input_buf: Buffer) -> None:
        print ("In TLS: class Context client client handle certificate")
        ...
        print ("In TLS: class Context client client handle certificate loaded x509 cert with chain") 
    ...
    def _client_handle_certificate_verify(self, input_buf: Buffer) -> None:
        print ("In TLS: class Context client client handle certificate verify")
    ...
    def _client_handle_finished(self, input_buf: Buffer, output_buf: Buffer) -> None:
        ...
        print ("In TLS: class Context client client handle certificate finished check verified data")
        # check verify data
        ...
        print ("In TLS: class Context client client handle certificate finished prepare trafic keys")
        ...
        # send finished
        print ("In TLS: class Context client client handle certificate finished check verified send finished")
        ...
        print ("In TLS: class Context client client handle certificate finished check verified commit trafic keys")
        # commit traffic key
    ...
    def _client_handle_new_session_ticket(self, input_buf: Buffer) -> None:
        print ("In TLS: class Context client client handle new session ticket")
    ...
    def _server_handle_hello(
        ...
        print ("In TLS: class Context client server handle hello negotiate parameters")
        cipher_suite = negotiate(
            self._cipher_suites,
        ...
        print ("\nprinting negotiated signature algorithm")
        ...
        if (
            ...
            if (
                ...
                print ("In TLS: class Context client server handle hello validate session ticket")
                ...
                if peer_hello.early_data:
                    print ("In TLS: class Context client server handle hello calculate early data key")
        ...
        # perform key exchange
        print ("In TLS: class Context client server handle hello perform key exchange")
        public_key: Union[
            ec.EllipticCurvePublicKey, x25519.X25519PublicKey, x448.X448PublicKey
        ]
        shared_key: Optional[bytes] = None
        for key_share in peer_hello.key_share:
            ...
            if isinstance(peer_public_key, x25519.X25519PublicKey):
                print ("In TLS: class Context client server handle hello perform key exchange X25519PublicKey")
                self._x25519_private_key = x25519.X25519PrivateKey.generate()
                public_key = self._x25519_private_key.public_key()
                shared_key = self._x25519_private_key.exchange(peer_public_key)
                break
            elif isinstance(peer_public_key, x448.X448PublicKey):
                print ("In TLS: class Context client server handle hello perform key exchange X448PublicKey")
                self._x448_private_key = x448.X448PrivateKey.generate()
                public_key = self._x448_private_key.public_key()
                shared_key = self._x448_private_key.exchange(peer_public_key)
                break
            elif isinstance(peer_public_key, ec.EllipticCurvePublicKey):
                print ("In TLS: class Context client server handle hello perform key exchange Elliptic curve public key")
                self._ec_private_key = ec.generate_private_key(
        ...
        with push_message(self.key_schedule, handshake_buf):
            print ("In TLS: class Context client server handle hello sending encrypted extentions")
        ...
        if pre_shared_key is None:
            # send certificate
            print ("In TLS: class Context client server handle hello if preshared key is none")
            with push_message(self.key_schedule, handshake_buf):
                push_certificate(
                    handshake_buf,
                    Certificate(
            ...
            print ("In TLS: class Context client server handle hello send certificate verify")
        ...
        if self.new_session_ticket_cb is not None and psk_key_exchange_mode is not None:
            print ("In TLS: class Context client server handle hello creating session ticket")
    ...
    def _server_handle_finished(self, input_buf: Buffer, output_buf: Buffer) -> None:
        finished = pull_finished(input_buf)
        print ("In TLS: class Context client server handle hello server handle finished")
        # check verify data
    ...
    def _setup_traffic_protection(
        ...
        print ("In TLS: class Context client server handle hello setup trafic protection")
